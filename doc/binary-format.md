...
**형식 버전은 하나뿐입니다.** 파일 머리의 버전 정수가 테이블 리더가 아는 값과 다르면 그 자리에서 멈춥니다. 옛 배치로 가는 호환 경로는 없고 만들 계획도 없습니다 — 이 빌드가 못 읽는 파일은 해석할 파일이 아니라 다시 뽑을 파일입니다. 지금 버전은 **102**이고, 디스크립터가 인코딩 바이트를 갖게 되면서 101을 통째로 대체했습니다.
...
핵심 결정은 **행 지향이 아니라 컬럼 지향**입니다. 그 선택으로 무엇을 얻고 무엇을 포기했는지, 어떤 상황에 맞고 어떤 상황에 틀린지는 [왜 컬럼 지향인가](../spec/scb-column-oriented-rationale.md)에 따로 적었습니다. 컬럼마다 값이 연속 블록이고 그 블록의 바이트 길이가 헤더에 있으므로, **모르는 컬럼 건너뛰기 = `advance(길이)` 한 번**입니다. 타입별 스킵 로직이 없으니 13개 테이블 리더가 각자 다르게 틀릴 자리도 없습니다.
...
```
fixed32    version        = 102
fixed8     flags          = 0        bit0 = 암호화, bit1 = 압축. 둘 다 아직 0
counter32  rowCount
...
columnCount ×:                        ── 데이터 ──
  블록: encoding이 정한 배치로 담긴 rowCount개의 값
```

디스크립터는 컬럼당 **8바이트**(태그와 count가 1바이트로 끝나는 보통의 경우)이고, **테이블당 한 번**입니다. 행이 늘어도 늘지 않습니다.
...
`SecondTable`(1행 3컬럼, 39바이트)입니다. 이 바이트열은 테스트에 그대로 박혀 있습니다 — [검증 게이트](#검증-게이트)의 형식 고정 항목.
...
한 행짜리 테이블에서도 인코딩 선택이 그대로 돕니다 — 정수 두 컬럼은 varint가 고정폭을 이기고, 문자열 컬럼은 사전을 만들어 봐야 원문보다 커지므로 RAW가 남습니다.
...
위까지가 **값 하나**를 어떻게 적느냐이고, 여기부터는 **한 컬럼의 값들**을 어떻게 적느냐입니다. 값마다 조밀하게 적어도 값 **사이**의 중복은 그대로 남는데, 정적 기획 데이터에서는 그 중복이 파일의 대부분입니다 — 같은 문자열이 수만 번, 1씩 증가하는 id, 유니크 값이 셋뿐인 enum 컬럼.

컬럼 지향 배치가 이미 같은 성질의 값을 한 블록에 모아 두었으므로, 인코딩은 그 블록 안에서만 닫히면 됩니다. 디스크립터의 `encoding` 바이트가 어느 배치인지 말합니다.

|번호|이름|블록 배치|어디에|
|--|--|--|--|
|0|RAW|위 「값 인코딩」 그대로|전부. 배열과 i64·실수·bool·uuid는 항상 이것|
|1|VARINT|행마다 `counter32` 값|작은 값이 모인 `int` 컬럼|
|2|DELTA|첫 값, 그다음 `counter32` 차이들|증가하는 `int` 컬럼|
|3|RLE|`counter32` 런 길이 + `counter32` 값의 반복|같은 값이 이어지는 `int`·`enum` 컬럼|
|4|DELTA_RLE|첫 값, 그다음 차이 스트림의 RLE|등차 컬럼. id가 여기 걸립니다|
|5|DICT|`counter32` 사전 크기 + 사전 + 행마다 `counter32` 인덱스|유니크가 적은 `string` 컬럼|
|6|DICT_RLE|사전 + 인덱스 스트림의 RLE|위에 더해 같은 값이 이어지는 경우|

- **스칼라만** 인코딩합니다. 배열은 항상 RAW인데, 실제 데이터셋에서 스칼라 문자열·정수·enum이 전체 바이트의 98%를 넘기 때문입니다.
...
- 사전은 **첫 등장 순서**입니다. 정렬이 아니라 등장 순서인 것은, 출력을 결정적으로 유지하면서 한 번의 순회로 만들 수 있기 때문입니다.
- 리더는 표에 없는 (원소, 인코딩) 조합을 만나면 — string 컬럼에 DELTA 같은 — 필드 이름을 짚어 멈춥니다. 모르는 번호도 같습니다. **모르는 컬럼은 여전히 `advance(byteLength)` 하나**입니다: 인코딩을 몰라도 건너뛰기는 성립합니다.

### 고르는 방법: 전부 해보고 가장 작은 것

변환기는 통계로 추측하지 않습니다. **적용 가능한 인코딩을 전부 인코딩해 보고 가장 작은 것을 고릅니다.** 동률이면 낮은 번호가 이깁니다.

인코딩 시간은 이 형식이 신경 쓰지 않는 유일한 자원이고(읽는 쪽만 빠르면 됩니다), 실제로 잰 바이트 수는 틀리는 법이 없는 유일한 선택 기준입니다. 선택이 결정적이므로 같은 입력은 같은 바이트를 내고, 골든 트리와 형식 고정 테스트가 그 성질에 기댑니다.

### 읽는 쪽: 컬럼 커서

생성된 테이블 리더의 행 루프는 그대로 행 루프이고, **값을 꺼내는 자리만** 커서를 거칩니다. 델타가 어떻게 누적되는지, 런이 얼마나 남았는지, 사전 인덱스가 무엇을 가리키는지 아는 곳은 언어마다 그 커서 하나입니다.

사전 인코딩은 파일 크기와 별개의 이득을 하나 더 줍니다: **문자열 객체가 유니크 개수만큼만 생깁니다.** 유니크 값 3개짜리 컬럼이 103,395행이면 문자열 셋을 만들고 행마다 그 참조를 나눠 주므로, 줄어드는 것이 로드 비용만이 아니라 [상주 메모리](benchmark.md)입니다.

## 테이블 리더의 동작

로드 시작에 한 번 계획을 세우고, 그다음 행 루프는 값을 읽는 것 외에 아무 판단도 하지 않습니다.

1. **헤더와 디스크립터를 읽고, 파일 자신과 대조한다.** 블록은 헤더 뒤에 오는 전부이므로 `byteLength`의 합이 남은 바이트와 **정확히 같아야** 합니다. RAW 블록은 행 수보다 짧을 수도 없습니다(어느 컬럼이든 한 행이 최소 1바이트). 인코딩된 블록에는 그 하한이 없으므로 — 런 하나가 10만 행을 덮습니다 — 대신 디코드가 런 길이의 합과 사전 인덱스 범위를 검사합니다. 어긋나면 행을 할당하기 **전에** 멈춥니다 — 손상된 행 수가 20억 행 할당이 되는 경로가 여기서 끊깁니다.
...
그중 하나는 **이 형식이 막아주지 못합니다.** 컬럼에 태그 재사용이 있듯 enum에는 **레이블 값의 재사용**이 있습니다 — 레이블을 중간에 끼워 넣어 뒤의 값이 밀리면, 옛 데이터의 `3`이 어제와 다른 라벨을 가리킵니다. 파일에 실리는 것은 숫자뿐이라 컬럼 쪽처럼 거절할 근거가 없고, 베이스라인도 컬럼만 기록합니다. 태그 재사용을 변환기가 원천 차단하는 것과 달리 **이쪽은 규율로 막아야 합니다.**
...
인코딩이 들어온 뒤로는 오히려 반대쪽이 눈에 띕니다. 잘 압축되는 큰 테이블에서는 디스크립터가 파일의 **상당 부분**이 됩니다 — 103,395행 × 9컬럼짜리 `StatGrowth`가 5,435바이트이고 그중 디스크립터가 72바이트입니다. 그래도 테이블당 한 번인 것은 그대로이고, 그 72바이트가 아까워지는 지점은 없습니다.
...
|**형식 고정**|가장 작은 테이블 39바이트를 **스펙에서** 적어 테스트 소스에 박아둠. 골든 트리는 변환기 출력에서 기록되므로 형식이 움직이면 함께 움직이지만, 이쪽은 사람이 고쳐야 움직입니다|
|**인코딩 선택 고정**|적합성 코퍼스의 컬럼마다 어느 인코딩이 이기는지를 이름으로 단정. 코퍼스 데이터는 일곱 인코딩이 각각 어딘가에서 승자가 되도록 짜여 있고, 이 단정이 그 커버리지가 조용히 좁아지지 않게 잡습니다|
...
- **`flags`는 아직 0뿐**입니다. bit0(암호화)과 bit1(압축)의 자리는 정해 두었지만 아무것도 읽거나 쓰지 않습니다. 대칭키 암호화의 봉투 배치는 [스펙](../spec/scb-v102-column-encoding.md)에 확정되어 있고 구현만 남았습니다 — 자리를 미리 정해 둔 것은 나중에 붙일 때 형식 버전을 다시 올리지 않기 위해서입니다. 범용 압축은 컬럼 인코딩만으로 gzip한 JSON보다 작아진 뒤라 필요가 줄었습니다.
