...
누가 언제 무엇을 바꿨는지 셀 단위로 추적하고 브라우저로 확인하기.
...
- **배포 판정** — 그 구간을 라이브에 내보내려면 **데이터 패치와 코드 배포 중 무엇이** 나가야 하나. ([아래](#배포-판정--이-구간을-내보내려면-무엇이-나가야-하나))
...
|`deployment`|**배포 판정.** 스냅샷마다 하나, 문서 전체에 하나. 바로 아래에서 설명합니다.|

### 배포 판정 — 이 구간을 내보내려면 무엇이 나가야 하나

히스토리의 모든 항목이 조용히 던지는 질문이 하나 있습니다 — **"그래서 뭘 배포해야 하지?"** 셀 수정은 데이터 패치로 나가지만, 상수는 빌드 말고는 어디에도 실리지 않고, enum은 이름이 코드에·숫자가 데이터에 나뉘어 있습니다. 이 답은 [무엇이 코드로 나가고 무엇이 데이터로 나가는지](languages/readme.md#데이터만-나가도-되는-변경과-코드가-함께-나가야-하는-변경)를 이미 아는 사람만 변경 목록에서 읽어낼 수 있습니다.

그래서 **히스토리가 대신 판정합니다.** 스냅샷마다, 그리고 범위 전체에 대해.
...
웹 페이지에서는 커밋마다 `DATA` / `CODE` 칩이 붙고(이유는 툴팁), Changes 카드 머리에 **범위 전체의 판정**이 나옵니다. JSON에서는 스냅샷마다 `deployment`, 문서 전체에 `deployment`가 하나 더 있습니다 — 범위 판정은 스냅샷 판정들의 **합집합**입니다. 중간의 한 스냅샷이 코드 배포를 요구하면 그 범위 전체가 요구하는 것이기 때문입니다.
...
판정 규칙은 [언어별 가이드의 표](languages/readme.md#데이터만-나가도-되는-변경과-코드가-함께-나가야-하는-변경) 그대로입니다.
...
|enum 레이블 (어느 컬럼도 안 쓸 때)|`code` — 데이터에 값이 실린 적이 없으므로 경고 없음|
|상수 세트|`code` — **데이터 패치는 이 변경을 아무것도 실어 나르지 못합니다**|

`warnings`는 그중에서도 **아무것도 실패하지 않는 변경**만 모읍니다. 변환도 성공하고 로딩도 성공하는데 결과가 틀리는 것들 — 파이프라인의 다른 어떤 도구도 이것을 말해줄 위치에 있지 않아서, 여기서 말합니다.

|경고가 붙는 변경|왜|
|--|--|
|enum 레이블 값이 밀림|이미 내보낸 데이터의 숫자가 **다른 라벨을 가리키게 됩니다.** 전체 재익스포트 + 코드와 함께 배포|
|enum 레이블 삭제|데이터만 롤백하면 이 빌드가 이름을 모르는 값이 되살아납니다|
|레이블 추가 + 같은 변환에서 데이터 변경|새 값을 쓰는 행이 있다면, 그 데이터가 구버전 빌드에 닿기 전에 **코드부터** 배포|
|테이블 삭제|그 전에 생성된 빌드는 로드할 때 여전히 그 파일을 찾습니다|

알아둘 것 셋.

- **판정은 저장되지 않고 읽을 때 계산됩니다.** 규칙이 좋아지면 과거 스냅샷의 판정도 함께 좋아집니다. 이 기능이 생기기 전에 기록된 스냅샷에도 판정이 붙는 이유입니다.
- **어느 컬럼도 쓰지 않는 enum은 데이터 쪽 판정에서 빠집니다.** 값이 어느 행에도 실린 적이 없으니, 값을 다시 매겨도 코드 수정일 뿐입니다. 다만 쓰이는지는 브랜치의 **현재 컬럼**으로 판단합니다 — 판정은 "지금 이걸 내보내면"에 대한 답이기 때문입니다.
- **정리(prune)된 스냅샷은 판정하지 않습니다.** 증거가 지워진 스냅샷에 "내보낼 것 없음"이라고 답하면 그건 다른 질문에 대한 오답입니다. `deployment`가 `null`이면 판정 불가이지 "없음"이 아닙니다.
