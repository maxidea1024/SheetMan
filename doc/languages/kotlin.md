...
recipe에 `"WriteUpdater": true`를 적으면 `sheetman/SheetManUpdater.kt`가 함께 나옵니다. CDN이나 버킷에 올려둔 데이터를 받아 로컬 사본을 최신으로 유지하는 코드이고, **배포를 다시 하지 않고 데이터만 패치**하기 위한 것입니다. 기본값이 `false`인 이유는 네트워크를 쓰는 유일한 생성물이기 때문입니다.
...
val options = SheetManUpdater.Options()
options.log = ::println

val result = SheetManUpdater.update("https://cdn.example.com/data", "./data", options)
