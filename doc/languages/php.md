...
**오토로더가 없습니다.** 이 생성물은 Composer를 전제하지 않으므로 각 파일이 자기가 쓰는 것을 `require_once`합니다 — 테이블 파일은 테이블 리더와 자기 필드가 쓰는 enum을, 접근자는 전부를. 그래서 소비자는 파일 하나만 require하면 됩니다. Composer 프로젝트에 넣어도 충돌하지 않습니다.

**`bigint`는 `int`입니다.** PHP의 `int`는 64비트 플랫폼에서 64비트입니다. 테이블 리더는 `unpack('P')`를 쓰지 않는데, 그것이 2^63을 넘으면 float가 되기 때문입니다 — 32비트 두 개로 나눠 조립합니다.
