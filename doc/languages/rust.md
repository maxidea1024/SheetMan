...
|크레이트 의존성|**기본값에서는 없음.** 리더는 core와 std만 씁니다 — 레지스트리 접근 없이 빌드됩니다. `WriteUpdater`를 켜면 `ureq` 하나가 붙습니다 (아래)|
...
`Result`를 돌려주지 않습니다. 네트워크·디스크·손상된 파일은 모두 호출한 쪽이 다뤄야 할 상황이지 결함이 아니기 때문이고, 원하는 답은 「안 됐고, 이유는 이것이고, 이전 데이터는 그대로 있다」이지 match해야 할 에러가 아니기 때문입니다. 받은 파일은 전부 매니페스트의 MD5와 대조하고, `.staging`을 거쳐 마지막에 한 번에 옆깁니다.
