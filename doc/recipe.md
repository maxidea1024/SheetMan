...
|`rescue`|마커 없이 **시트 탭 이름이 곧 테이블**이고 머리 3줄이 헤더인 형태. 다른 규칙으로 작성된 시트를 그대로 읽기 위한 것입니다.|
...
뒤의 둘은 원본을 고칠 수 없는 동안 나머지를 변환하기 위한 것이고, `rescue` 레이아웃에서만 동작합니다. 버린 행은 전부 로그에 남습니다.
...
```json
"Targets": [
  { "Type": "binary", "Path": "./out/data", "FileExtension": ".scb" },
...
|`BinaryTableFileExtension`|`".scb"`|**생성된 테이블 리더가 찾을 데이터 파일의 확장자.** 익스포터의 `FileExtension`과 **반드시 같아야** 합니다 — 다르면 테이블 리더가 파일을 못 찾습니다. 유니티에 넣는다면 `.bytes`가 필요할 수 있습니다.|
...
|키|해당|기본값|무엇인가|
|--|--|--|--|
|`FileExtension`|Binary|`".scb"`|각 테이블 파일의 확장자. 코드 생성 쪽 `BinaryTableFileExtension`과 짝을 맞추세요|
|`Compress`|Binary|`false`|**예약. 구현되어 있지 않습니다.** 형식이 압축 플래그 자리를 비워두고 있을 뿐, 아무것도 읽거나 쓰지 않습니다|
|`SchemaBaseline`|Binary|`""`|지난 스키마의 기록을 둘 경로. **커밋하세요.** 매 실행이 스키마를 그것과 비교해서, 이미 배포된 테이블 리더가 버티지 못할 변경이면 **아무것도 쓰기 전에** 컬럼 이름을 짚어 멈춥니다. 비워두면 검사하지 않습니다|
...
`sheets/`의 워크북을 읽어 `generated/data/<테이블>.scb`와 `generated/cs/`의 C# 코드를 냅니다.
...
|`table-extension.json`|`.scb`가 아닌 확장자로 맞추기|
    ...
    "Xlsx": [
      {
        ...
        // 시트를 읽는 방식. 기본은 `sheetman` — 마커로 엔티티를 선언하는 방식입니다.
