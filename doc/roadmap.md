...
- **스큐 하네스를 나머지 12개 언어로.** [스키마 진화](binary-format-v101.md)의 교차 읽기 게이트가 지금 C#만 돌립니다. 값 읽기는 적합성 코퍼스가 전 언어를 덮지만, 모르는 컬럼 건너뛰기와 타입 승격을 **실제로** 확인한 것은 C#입니다.
...
- 바이너리 형식 v101 — 컬럼 지향·태그로 스키마 진화 ([문서](binary-format-v101.md)), 리더 13개 재작성, 스큐 코퍼스(C#), 형식 바이트 고정, 배포 전 베이스라인 검사
