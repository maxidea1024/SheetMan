...
- `foreign[]`은 지원하지 않습니다. 로우마다 가변 개수의 참조를 해석해야 하는데 생성되는 테이블 리더에 그런 형태가 없어서, 조용히 해석되지 않는 코드를 만드는 대신 명시적으로 거부합니다. 고정 개수라면 `SerialField`를 사용하세요.
...
두 방식은 **와이어 포맷이 다릅니다**. `SerialField`는 길이가 생성 시점에 알려져 있으므로 바이너리에 길이를 기록하지 않고, 구분자 배열은 로우마다 길이를 기록합니다. 생성되는 테이블 리더가 이를 구분해서 처리합니다.
...
지금까지 설명한 것이 **`sheetman` 레이아웃**이고 기본값입니다.

`rescue`는 **다른 규칙으로 작성된 시트를 그대로 읽는** 레이아웃입니다. 시트를 고칠 필요가 없고, 여기서 나오는 코드·바이너리·문서는 `sheetman` 레이아웃과 동일합니다.
...
`ArrayDelimiter`는 recipe 전체 기본값이 있고, **소스 항목에 적으면 그쪽이 우선합니다.** 두 벌의 시트가 서로 다른 구분자로 쓰여 있어도 한 recipe에서 같이 읽을 수 있습니다.

```jsonc
"Xlsx": [
  { "Path": "./sheets",       "Layout": "sheetman" },                        // recipe 기본값(`;`)
  { "Path": "./other-sheets", "Layout": "rescue", "ArrayDelimiter": "|" }    // 이 항목만 `|`
]
```
...
- **한 레시피에서 섞어 읽을 수 있습니다.** 소스 항목마다 `Layout`을 따로 주면 되고, 한쪽 워크북의 테이블이 다른 쪽에서 선언한 enum을 타입으로 쓸 수 있습니다.
