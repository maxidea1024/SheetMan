 ...
 * libcurl
 *
 * This is the one file SheetMan emits that is not self-contained. C has no HTTP
 ...
 * Define SHEETMAN_UPDATER_IMPLEMENTATION in exactly one translation unit; the
 ...
 *
 * Zero-initialise and call sm_update_options_init, which fills in the working
 * defaults; a struct of zeroes would mean no attempts and no timeout. */
typedef struct sm_update_options {
...
void sm_update_options_init(sm_update_options* options);

/* What an update did. */
typedef struct sm_update_result {
  bool succeeded;
  bool up_to_date;
...
bool sm_update(const char* base_url, const char* cache_directory,
               const sm_update_options* options, sm_update_result* result);
...
void sm_md5_hex(const uint8_t* data, size_t length, char out[33]);

#ifdef __cplusplus
}
#endif

#endif /* SHEETMAN_UPDATER_H */

/* ======================================================================== */

#ifdef SHEETMAN_UPDATER_IMPLEMENTATION
...
#include <direct.h>
#include <windows.h>
#define sm_mkdir(path) _mkdir(path)
...
static uint32_t sm_md5_rotate(uint32_t value, uint32_t bits) {
...
typedef struct sm_manifest_entry {
  char* name;
  int64_t size;
  char hash[33];
} sm_manifest_entry;

typedef struct sm_manifest {
  sm_manifest_entry* entries;
  int32_t count;
} sm_manifest;

static void sm_manifest_free(sm_manifest* manifest) {
  int32_t at;
 ...
 * defeat this. */
typedef struct sm_json {
  const char* text;
  size_t length;
  size_t at;
} sm_json;

static void sm_json_skip_space(sm_json* json) {
...
static char sm_json_peek(const sm_json* json) {
...
static bool sm_json_read_string(sm_json* json, char* out, size_t out_size) {
  size_t written = 0;

  if (sm_json_peek(json) != '"')
    return false;
...
static bool sm_json_skip_value(sm_json* json);

static bool sm_json_skip_container(sm_json* json, char close) {
  json->at++; /* the opening brace or bracket */
  sm_json_skip_space(json);

  if (sm_json_peek(json) == close) {
    json->at++;
    return true;
  }
  ...
  for (;;) {
    ...
    if (close == '}') {
      if (!sm_json_read_string(json, NULL, 0))
        return false;
...
static bool sm_json_skip_value(sm_json* json) {
  char c = sm_json_peek(json);

  if (c == '{')
    return sm_json_skip_container(json, '}');

  if (c == '[')
    return sm_json_skip_container(json, ']');

  if (c == '"')
    return sm_json_read_string(json, NULL, 0);
...
static bool sm_json_read_entry(sm_json* json, sm_manifest_entry* entry) {
  char name[512];
  ...
  for (;;) {
    ...
    if (!sm_json_read_string(json, key, sizeof key))
      return false;
    ...
    if (strcmp(key, "Name") == 0) {
      if (!sm_json_read_string(json, name, sizeof name))
        return false;
    } else if (strcmp(key, "Hash") == 0) {
      if (!sm_json_read_string(json, entry->hash, sizeof entry->hash))
        return false;
    ...
    } else if (!sm_json_skip_value(json)) {
      return false;
    }
...
static bool sm_parse_manifest(const char* text, size_t length, sm_manifest* out) {
  sm_json json;
  int32_t capacity = 16;
  ...
  out->entries = (sm_manifest_entry*)malloc((size_t)capacity * sizeof *out->entries);
  ...
  for (;;) {
    sm_manifest_entry entry;

    sm_json_skip_space(&json);

    if (!sm_json_read_entry(&json, &entry)) {
      sm_manifest_free(out);
      return false;
    }

    if (entry.name != NULL) {
      if (out->count == capacity) {
        sm_manifest_entry* grown;

        capacity *= 2;
        grown = (sm_manifest_entry*)realloc(out->entries,
...
static bool sm_local_path(char* out, size_t out_size, const char* directory,
                          const char* name) {
  size_t at = 0;
  size_t i;
  ...
  for (i = 0; name[i] != '\0'; ++i)
    out[at++] = name[i] == '/' ? SM_SEPARATOR : name[i];
 ...
 * failure. */
static bool sm_make_directory(const char* path) {
  char buffer[1024];
...
static bool sm_parent_of(char* out, size_t out_size, const char* path) {
  size_t length = strlen(path);
...
static FILE* sm_fopen(const char* filename, const char* mode) {
#if defined(_MSC_VER)
  FILE* file = NULL;
...
static bool sm_file_exists(const char* path) {
  FILE* file = sm_fopen(path, "rb");
...
static bool sm_write_file(const char* path, const uint8_t* data, size_t length) {
  FILE* file = sm_fopen(path, "wb");
  size_t written;
...
typedef struct sm_body {
  uint8_t* data;
  size_t length;
  size_t capacity;
} sm_body;

static size_t sm_body_write(char* chunk, size_t size, size_t count, void* context) {
  sm_body* body = (sm_body*)context;
...
static void sm_body_free(sm_body* body) {
  free(body->data);
 ...
 * will answer. */
static bool sm_join_url(char* out, size_t out_size, const char* base_url,
                        const char* name) {
...
static void sm_log(const sm_update_options* options, const char* message) {
  if (options->log != NULL)
    options->log(options->log_context, message);
}

/* One request. `transient` says whether trying again could go differently. */
static bool sm_fetch(const char* url, const sm_update_options* options,
                     sm_body* body, bool* transient, char* error, size_t error_size) {
  ...
  *transient = false;
  ...
  curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, sm_body_write);
...
static bool sm_download(const char* url, const sm_update_options* options,
                        sm_body* body, char* error, size_t error_size) {
  ...
  for (attempt = 1; ; ++attempt) {
    ...
    if (sm_fetch(url, options, body, &transient, error, error_size))
      return true;
    ...
    {
      char message[SHEETMAN_ERROR_MAX + 64];

      snprintf(message, sizeof message, "sheetman: %s Retrying in %.1fs (%d of %d).",
               error, (double)delay / 1000.0, (int)attempt, (int)attempts);

      sm_log(options, message);
    }
...
static void sm_read_local_manifest(const char* path, sm_manifest* out) {
  uint8_t* bytes = NULL;
  int32_t length = 0;
  ...
  if (!sm_read_all_bytes(path, &bytes, &length))
    return;

  if (!sm_parse_manifest((const char*)bytes, (size_t)length, out)) {
    out->entries = NULL;
    out->count = 0;
  }
...
static const sm_manifest_entry* sm_find_entry(const sm_manifest* manifest,
                                              const char* name) {
  int32_t at;
...
static void sm_remove_staged(const char* staging, const char* name) {
  char path[1024];
  char parent[1024];

  if (!sm_local_path(path, sizeof path, staging, name))
    return;

  remove(path);

  while (sm_parent_of(parent, sizeof parent, path) != false
         && strcmp(parent, staging) != 0) {
    if (sm_rmdir(parent) != 0)
      return;
...
bool sm_update(const char* base_url, const char* cache_directory,
  ...
  if (options == NULL) {
    sm_update_options_init(&defaults);
    options = &defaults;
  }
  ...
  if (!sm_join_url(url, sizeof url, base_url, options->manifest_file_name)) {
    snprintf(result->error, sizeof result->error, "the base URL is too long.");
    return false;
  }

  if (!sm_download(url, options, &body, result->error, sizeof result->error)) {
    sm_body_free(&body);
    goto failed;
  }
  ...
  if (!sm_parse_manifest(manifest_text, manifest_length, &remote)) {
    snprintf(result->error, sizeof result->error,
             "the manifest could not be read: it has no Items array.");
    goto failed;
  }

  if (!sm_local_path(path, sizeof path, cache_directory, options->manifest_file_name)) {
    snprintf(result->error, sizeof result->error, "the cache path is too long.");
    goto failed;
  }

  sm_read_local_manifest(path, &local);
...

  {
    ...
    for (at = 0; at < remote.count; ++at) {
      const sm_manifest_entry* previous = sm_find_entry(&local, remote.entries[at].name);
      bool current;

      if (!sm_local_path(path, sizeof path, cache_directory, remote.entries[at].name)) {
        snprintf(result->error, sizeof result->error, "the cache path is too long.");
        goto failed;
      }
    ...
    for (at = 0; at < local.count; ++at) {
      if (sm_find_entry(&remote, local.entries[at].name) == NULL)
        gone++;
    }

    if (wanted == 0 && gone == 0) {
      sm_log(options, "sheetman: already up to date.");

      result->succeeded = true;
      result->up_to_date = true;

      free(manifest_text);
      sm_manifest_free(&remote);
      sm_manifest_free(&local);
      sm_body_free(&body);
      return true;
    }

    {
      ...
      snprintf(message, sizeof message,
               "sheetman: %d file(s) to fetch, %d to remove.", (int)wanted, (int)gone);
  ...
  if (!sm_local_path(staging, sizeof staging, cache_directory, ".staging")) {
    snprintf(result->error, sizeof result->error, "the cache path is too long.");
    goto failed;
  }

  sm_make_directory(cache_directory);
  sm_make_directory(staging);

  for (at = 0; at < remote.count; ++at) {
    const sm_manifest_entry* entry = &remote.entries[at];
    const sm_manifest_entry* previous = sm_find_entry(&local, entry->name);
    char parent[1024];

    if (!sm_local_path(path, sizeof path, cache_directory, entry->name))
      continue;
    ...
    if (!sm_join_url(url, sizeof url, base_url, entry->name)) {
    ...
    if (options->verify_hash && entry->hash[0] != '\0') {
      ...
      sm_md5_hex(body.data, body.length, actual);
    ...
    if (!sm_local_path(path, sizeof path, staging, entry->name)) {
    ...
    if (sm_parent_of(parent, sizeof parent, path))
      sm_make_directory(parent);

    if (!sm_write_file(path, body.data, body.length)) {
  ...
  if (failed) {
    ...
    for (undo = 0; undo < remote.count; ++undo)
      sm_remove_staged(staging, remote.entries[undo].name);
  ...
  for (at = 0; at < local.count; ++at) {
    ...
    if (sm_local_path(path, sizeof path, cache_directory, local.entries[at].name))
      remove(path);
  ...
  for (at = 0; at < remote.count; ++at) {
    ...
    if (!sm_local_path(staged_path, sizeof staged_path, staging, remote.entries[at].name))
      continue;

    if (!sm_file_exists(staged_path))
      continue;
  ...
  if (!sm_local_path(path, sizeof path, cache_directory, options->manifest_file_name)
      || !sm_write_file(path, (const uint8_t*)manifest_text, manifest_length)) {
    snprintf(result->error, sizeof result->error,
  ...
  for (at = 0; at < remote.count; ++at)
    sm_remove_staged(staging, remote.entries[at].name);
...

  {
    ...
    snprintf(message, sizeof message, "sheetman: updated. %d fetched, %d removed.",
...
failed:
  {
    char message[SHEETMAN_ERROR_MAX + 32];

    snprintf(message, sizeof message, "sheetman: update failed: %s", result->error);
    sm_log(options, message);
  }
...
#endif /* SHEETMAN_UPDATER_IMPLEMENTATION */
