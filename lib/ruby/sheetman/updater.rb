  ...
  def self.update(base_url, cache_directory, options = UpdateOptions.new)
    ...
    begin
      ...
      if wanted.empty? && gone.empty?
        log.call('sheetman: already up to date.')
      ...
      log.call("sheetman: #{wanted.length} file(s) to fetch, #{gone.length} to remove.")
      ...
      log.call("sheetman: updated. #{result.downloaded_count} fetched, " \
      ...
      log.call("sheetman: update failed: #{result.error}")
      result
    end
  end
  ...
  def self.download(url, options, log)
    ...
    (1..attempts).each do |attempt|
      begin
        ...
        log.call(format('sheetman: %s Retrying in %.1fs (%d of %d).',
