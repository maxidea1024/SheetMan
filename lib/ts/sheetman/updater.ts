...
export async function update(
  ...
  try {
    ...
    log(`[sheetman] ${wanted.length} file(s) to fetch, ${gone.length} to remove.`)
    ...
    log(`[sheetman] Updated. ${result.downloadedCount} fetched, ${result.deletedCount} removed.`)
    ...
    log(`[sheetman] Update failed: ${result.error}`)
    return result
  }
  ...
  async function download(url: string): Promise<Uint8Array> {
    ...
    for (let attempt = 1; ; attempt++) {
      try {
        ...
        log(`[sheetman] ${(e as Error).message} Retrying in ${delay}ms ` +
