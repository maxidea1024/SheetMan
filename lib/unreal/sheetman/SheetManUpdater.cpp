// The updater's implementation. See SheetManUpdater.h for what it promises.
...
#define SHEETMAN_TICKER FTicker
#else
#define SHEETMAN_TICKER FTSTicker
#endif
...
FString FSheetManUpdater::DefaultCacheDirectory()
{
    return FPaths::Combine(FPaths::ProjectPersistentDownloadDir(), TEXT("SheetManData"));
}

FString FSheetManUpdater::HashOf(const TArray<uint8>& Bytes)
{
    uint8 Digest[16];
...
void FSheetManUpdater::ParseManifest(const FString& Json, TArray<FSheetManManifestEntry>& OutEntries)
{
    OutEntries.Reset();
...
TSharedRef<FSheetManUpdater> FSheetManUpdater::Update(
    const FString& InBaseUrl,
    const FString& InCacheDirectory,
    const FSheetManUpdateOptions& InOptions,
    FSheetManUpdateComplete InOnComplete)
{
    TSharedRef<FSheetManUpdater> Updater = MakeShared<FSheetManUpdater>();
...
void FSheetManUpdater::FetchManifest()
{
...
void FSheetManUpdater::OnManifestFetched(const FString& Text)
{
    RemoteManifestText = Text;

    TArray<FSheetManManifestEntry> Remote;
    ParseManifest(Text, Remote);

    TArray<FSheetManManifestEntry> Local;
    FString LocalText;
    ...
    for (const FSheetManManifestEntry& Entry : Remote)
    {
        const FSheetManManifestEntry* Previous = Local.FindByPredicate(
            [&Entry](const FSheetManManifestEntry& One) { return One.Name == Entry.Name; });
    ...
    for (const FSheetManManifestEntry& Entry : Local)
    {
    ...
    if (Wanted.Num() == 0 && Gone.Num() == 0)
    {
        ...
        UE_LOG(LogTemp, Log, TEXT("SheetManUpdater: already up to date."));
        Finish();
        return;
    }

    UE_LOG(LogTemp, Log, TEXT("SheetManUpdater: %d file(s) to fetch, %d to remove."),
        Wanted.Num(), Gone.Num());
...
void FSheetManUpdater::FetchNext()
{
    if (At >= Wanted.Num())
    {
        Apply();
...
void FSheetManUpdater::OnFileFetched(const TArray<uint8>& Bytes)
{
    const FSheetManManifestEntry& Entry = Wanted[At];
...
void FSheetManUpdater::Apply()
{
    ...
    for (const FSheetManManifestEntry& Entry : Wanted)
    {
        const FString Staged = FPaths::Combine(StagingDirectory, Entry.Name);
    ...
    UE_LOG(LogTemp, Log, TEXT("SheetManUpdater: updated. %d fetched, %d removed."),
...
void FSheetManUpdater::Send()
{
    TSharedRef<FSheetManUpdater> This = AsShared();
...
void FSheetManUpdater::Retry()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("SheetManUpdater: '%s' failed. Retrying in %.1fs (%d of %d)."),
    ...
    SHEETMAN_TICKER::GetCoreTicker().AddTicker(
...
void FSheetManUpdater::Fail(const FString& InError)
{
    // The previous data is untouched, so the caller can carry on with it.
    UE_LOG(LogTemp, Error, TEXT("SheetManUpdater: %s"), *InError);
