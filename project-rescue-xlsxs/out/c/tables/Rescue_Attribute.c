...
#include <string.h>
static bool Rescue_AttributeParse(Rescue_AttributeTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_AttributeRecord_t*)sm_arena_alloc(
...
bool Rescue_AttributeLoad(Rescue_AttributeTable_t* table, const char* filename,
  ...
  Rescue_AttributeTable_t loaded;
