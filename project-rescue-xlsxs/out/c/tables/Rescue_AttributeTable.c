...
#include <string.h>
static bool Rescue_AttributeTableParse(Rescue_AttributeTableTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_AttributeTableRecord_t*)sm_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    Rescue_AttributeTableRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 1:
      (void)sm_check_column(reader, column, "AttributeTable.Id", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 2:
      (void)sm_check_column(reader, column, "AttributeTable.Name", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 3:
      (void)sm_check_column(reader, column, "AttributeTable.AttributeName", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 4:
      (void)sm_check_column(reader, column, "AttributeTable.AttributeType", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 5:
      (void)sm_check_column(reader, column, "AttributeTable.TargetAttributeType", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 6:
      (void)sm_check_column(reader, column, "AttributeTable.DamageValueRate", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_F32));
    ...
    case 7:
      (void)sm_check_column(reader, column, "AttributeTable.IsVisible", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_BOOL));
    ...
    case 8:
      (void)sm_check_column(reader, column, "AttributeTable.IconPath", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
...
bool Rescue_AttributeTableLoad(Rescue_AttributeTableTable_t* table, const char* filename,
  ...
  Rescue_AttributeTableTable_t loaded;
...
void Rescue_AttributeTableFree(Rescue_AttributeTableTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_AttributeTableRecord_t* Rescue_AttributeTableFindById(
  const Rescue_AttributeTableTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_AttributeTableContainsId(
  const Rescue_AttributeTableTable_t* table, int32_t key) {
