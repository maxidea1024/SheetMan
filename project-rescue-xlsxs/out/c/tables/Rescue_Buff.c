...
#include <string.h>
static bool Rescue_BuffParse(Rescue_BuffTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_BuffRecord_t*)sm_arena_alloc(
...
bool Rescue_BuffLoad(Rescue_BuffTable_t* table, const char* filename,
...
void Rescue_BuffFree(Rescue_BuffTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_BuffRecord_t* Rescue_BuffFindById(
  const Rescue_BuffTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_BuffContainsId(
  const Rescue_BuffTable_t* table, int32_t key) {
