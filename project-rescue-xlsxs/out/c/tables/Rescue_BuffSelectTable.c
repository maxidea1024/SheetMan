...
#include <string.h>
static bool Rescue_BuffSelectTableParse(Rescue_BuffSelectTableTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_BuffSelectTableRecord_t*)sm_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    Rescue_BuffSelectTableRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 1:
      (void)sm_check_column(reader, column, "BuffSelectTable.Id", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 2:
      (void)sm_check_column(reader, column, "BuffSelectTable.BuffName", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 3:
      (void)sm_check_column(reader, column, "BuffSelectTable.BuffID", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 4:
      (void)sm_check_column(reader, column, "BuffSelectTable.Grade", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 5:
      (void)sm_check_column(reader, column, "BuffSelectTable.BuffRate", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_F32));


      for (row = 0; row < table->count && !sm_failed(reader); ++row) {
        Rescue_BuffSelectTableRecord_t* record = &table->records[row];

        (void)sm_read_float(reader, &record->buff_rate);
      }

      break;

    case 6:
      (void)sm_check_column(reader, column, "BuffSelectTable.BuffTooltip", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 7:
      (void)sm_check_column(reader, column, "BuffSelectTable.IconPath", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
...
bool Rescue_BuffSelectTableLoad(Rescue_BuffSelectTableTable_t* table, const char* filename,
  ...
  Rescue_BuffSelectTableTable_t loaded;
