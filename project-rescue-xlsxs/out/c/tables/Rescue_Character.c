...
#include <string.h>
static bool Rescue_CharacterParse(Rescue_CharacterTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_CharacterRecord_t*)sm_arena_alloc(
...
bool Rescue_CharacterLoad(Rescue_CharacterTable_t* table, const char* filename,
  ...
  Rescue_CharacterTable_t loaded;
