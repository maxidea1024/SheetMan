...
#include <string.h>
static bool Rescue_ConfigParse(Rescue_ConfigTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_ConfigRecord_t*)sm_arena_alloc(
...
bool Rescue_ConfigLoad(Rescue_ConfigTable_t* table, const char* filename,
...
void Rescue_ConfigFree(Rescue_ConfigTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_ConfigRecord_t* Rescue_ConfigFindByIndex(
  const Rescue_ConfigTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_index, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_ConfigContainsIndex(
  const Rescue_ConfigTable_t* table, int32_t key) {
  return sm_index_find(table->by_index, table->count, key) >= 0;
}

const Rescue_ConfigRecord_t* Rescue_ConfigFindById(
  const Rescue_ConfigTable_t* table, const char* key) {
  int32_t position = sm_string_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_ConfigContainsId(
  const Rescue_ConfigTable_t* table, const char* key) {
