...
#include <string.h>
static bool Rescue_ConfigTableParse(Rescue_ConfigTableTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_ConfigTableRecord_t*)sm_arena_alloc(
    ...
    if (table->by_index == NULL)
      return sm_fail_with(reader, "out of memory allocating the Index index");

    table->by_id = (sm_string_index_entry*)sm_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    Rescue_ConfigTableRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 1:
      (void)sm_check_column(reader, column, "ConfigTable.Index", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 2:
      (void)sm_check_column(reader, column, "ConfigTable.Id", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 3:
      (void)sm_check_column(reader, column, "ConfigTable.Category", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 4:
      (void)sm_check_column(reader, column, "ConfigTable.DataType", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 5:
      (void)sm_check_column(reader, column, "ConfigTable.DefaultValue", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 6:
      (void)sm_check_column(reader, column, "ConfigTable.Description", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
  ...
  sm_string_index_sort(table->by_id, table->count);
...
bool Rescue_ConfigTableLoad(Rescue_ConfigTableTable_t* table, const char* filename,
  ...
  Rescue_ConfigTableTable_t loaded;
...
const Rescue_ConfigTableRecord_t* Rescue_ConfigTableFindByIndex(
  ...
  int32_t position = sm_index_find(table->by_index, table->count, key);
...
bool Rescue_ConfigTableContainsIndex(
  const Rescue_ConfigTableTable_t* table, int32_t key) {
  return sm_index_find(table->by_index, table->count, key) >= 0;
}

const Rescue_ConfigTableRecord_t* Rescue_ConfigTableFindById(
  const Rescue_ConfigTableTable_t* table, const char* key) {
  int32_t position = sm_string_index_find(table->by_id, table->count, key);
...
bool Rescue_ConfigTableContainsId(
  const Rescue_ConfigTableTable_t* table, const char* key) {
  return sm_string_index_find(table->by_id, table->count, key) >= 0;
}
