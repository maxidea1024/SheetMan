...
#include <string.h>
static bool Rescue_CostCurveTableParse(Rescue_CostCurveTableTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_CostCurveTableRecord_t*)sm_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    Rescue_CostCurveTableRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 1:
      (void)sm_check_column(reader, column, "CostCurveTable.Id", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 2:
      (void)sm_check_column(reader, column, "CostCurveTable.Name", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_STRING));
    ...
    case 3:
      (void)sm_check_column(reader, column, "CostCurveTable.GrowthType", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 4:
      (void)sm_check_column(reader, column, "CostCurveTable.CostType", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
    ...
    case 5:
      (void)sm_check_column(reader, column, "CostCurveTable.BaseCostValue", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));


      for (row = 0; row < table->count && !sm_failed(reader); ++row) {
        Rescue_CostCurveTableRecord_t* record = &table->records[row];

        (void)sm_read_i32_as(reader, column->element, &record->base_cost_value);
      }
...
bool Rescue_CostCurveTableLoad(Rescue_CostCurveTableTable_t* table, const char* filename,
  ...
  Rescue_CostCurveTableTable_t loaded;
