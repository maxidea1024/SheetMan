...
#include <string.h>
static bool Rescue_GachaArtifactListParse(Rescue_GachaArtifactListTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_GachaArtifactListRecord_t*)sm_arena_alloc(
...
bool Rescue_GachaArtifactListLoad(Rescue_GachaArtifactListTable_t* table, const char* filename,
  ...
  Rescue_GachaArtifactListTable_t loaded;
...
void Rescue_GachaArtifactListFree(Rescue_GachaArtifactListTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_GachaArtifactListRecord_t* Rescue_GachaArtifactListFindById(
  const Rescue_GachaArtifactListTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_GachaArtifactListContainsId(
  const Rescue_GachaArtifactListTable_t* table, int32_t key) {
