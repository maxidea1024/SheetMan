...
#include <string.h>
static bool Rescue_InfoGrowthParse(Rescue_InfoGrowthTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_InfoGrowthRecord_t*)sm_arena_alloc(
...
bool Rescue_InfoGrowthLoad(Rescue_InfoGrowthTable_t* table, const char* filename,
  ...
  Rescue_InfoGrowthTable_t loaded;
...
void Rescue_InfoGrowthFree(Rescue_InfoGrowthTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_InfoGrowthRecord_t* Rescue_InfoGrowthFindById(
  const Rescue_InfoGrowthTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_InfoGrowthContainsId(
  const Rescue_InfoGrowthTable_t* table, int32_t key) {
