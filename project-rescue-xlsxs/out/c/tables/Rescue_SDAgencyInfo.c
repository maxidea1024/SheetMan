...
#include <string.h>
static bool Rescue_SDAgencyInfoParse(Rescue_SDAgencyInfoTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDAgencyInfoRecord_t*)sm_arena_alloc(
...
bool Rescue_SDAgencyInfoLoad(Rescue_SDAgencyInfoTable_t* table, const char* filename,
  ...
  Rescue_SDAgencyInfoTable_t loaded;
...
void Rescue_SDAgencyInfoFree(Rescue_SDAgencyInfoTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_SDAgencyInfoRecord_t* Rescue_SDAgencyInfoFindById(
  const Rescue_SDAgencyInfoTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_SDAgencyInfoContainsId(
  const Rescue_SDAgencyInfoTable_t* table, int32_t key) {
