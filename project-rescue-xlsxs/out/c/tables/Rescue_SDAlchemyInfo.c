...
#include <string.h>
static bool Rescue_SDAlchemyInfoParse(Rescue_SDAlchemyInfoTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDAlchemyInfoRecord_t*)sm_arena_alloc(
...
bool Rescue_SDAlchemyInfoLoad(Rescue_SDAlchemyInfoTable_t* table, const char* filename,
  ...
  Rescue_SDAlchemyInfoTable_t loaded;
