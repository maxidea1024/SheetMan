...
#include <string.h>
static bool Rescue_SDContensInfoParse(Rescue_SDContensInfoTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDContensInfoRecord_t*)sm_arena_alloc(
...
bool Rescue_SDContensInfoLoad(Rescue_SDContensInfoTable_t* table, const char* filename,
  ...
  Rescue_SDContensInfoTable_t loaded;
...
void Rescue_SDContensInfoFree(Rescue_SDContensInfoTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_SDContensInfoRecord_t* Rescue_SDContensInfoFindById(
  const Rescue_SDContensInfoTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_SDContensInfoContainsId(
  const Rescue_SDContensInfoTable_t* table, int32_t key) {
