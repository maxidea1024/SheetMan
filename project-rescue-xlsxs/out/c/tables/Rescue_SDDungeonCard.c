...
#include <string.h>
static bool Rescue_SDDungeonCardParse(Rescue_SDDungeonCardTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDDungeonCardRecord_t*)sm_arena_alloc(
...
bool Rescue_SDDungeonCardLoad(Rescue_SDDungeonCardTable_t* table, const char* filename,
  ...
  Rescue_SDDungeonCardTable_t loaded;
...
void Rescue_SDDungeonCardFree(Rescue_SDDungeonCardTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_SDDungeonCardRecord_t* Rescue_SDDungeonCardFindById(
  const Rescue_SDDungeonCardTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_SDDungeonCardContainsId(
  const Rescue_SDDungeonCardTable_t* table, int32_t key) {
