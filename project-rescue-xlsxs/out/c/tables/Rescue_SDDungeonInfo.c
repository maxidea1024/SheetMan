...
#include <string.h>
static bool Rescue_SDDungeonInfoParse(Rescue_SDDungeonInfoTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDDungeonInfoRecord_t*)sm_arena_alloc(
...
bool Rescue_SDDungeonInfoLoad(Rescue_SDDungeonInfoTable_t* table, const char* filename,
  ...
  Rescue_SDDungeonInfoTable_t loaded;
...
void Rescue_SDDungeonInfoFree(Rescue_SDDungeonInfoTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_SDDungeonInfoRecord_t* Rescue_SDDungeonInfoFindById(
  const Rescue_SDDungeonInfoTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_SDDungeonInfoContainsId(
  const Rescue_SDDungeonInfoTable_t* table, int32_t key) {
