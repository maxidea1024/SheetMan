...
#include <string.h>
static bool Rescue_SDPubInfoParse(Rescue_SDPubInfoTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_SDPubInfoRecord_t*)sm_arena_alloc(
...
bool Rescue_SDPubInfoLoad(Rescue_SDPubInfoTable_t* table, const char* filename,
  ...
  Rescue_SDPubInfoTable_t loaded;
