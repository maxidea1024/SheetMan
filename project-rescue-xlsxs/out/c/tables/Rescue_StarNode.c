...
#include <string.h>
static bool Rescue_StarNodeParse(Rescue_StarNodeTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_StarNodeRecord_t*)sm_arena_alloc(
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 8:
      (void)sm_check_column(reader, column, "StarNode.UnlockCondition", SM_KIND_VAR_ARRAY, 0, SM_ELEMENT_MASK(SM_ELEMENT_I32) | SM_ELEMENT_MASK(SM_ELEMENT_VARINT));
