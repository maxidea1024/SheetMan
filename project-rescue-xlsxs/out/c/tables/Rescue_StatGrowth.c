...
#include <string.h>
static bool Rescue_StatGrowthParse(Rescue_StatGrowthTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_StatGrowthRecord_t*)sm_arena_alloc(
...
bool Rescue_StatGrowthLoad(Rescue_StatGrowthTable_t* table, const char* filename,
  ...
  Rescue_StatGrowthTable_t loaded;
