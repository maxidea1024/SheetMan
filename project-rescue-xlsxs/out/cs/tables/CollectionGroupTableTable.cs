...
{
    [System.Serializable]
    public partial class CollectionGroupTableTable
    {
        #region Record
        [System.Serializable]
        public partial class Record
        {
            ...
            /// </summary>
            public int[] CharacterList => _characterList;
            ...
            internal int _index;
            internal int[] _characterList = System.Array.Empty<int>();
            ...
            public override string ToString()
            {
                ...
                sb.Append(",\"CharacterList\":"); ToStringHelper.ToString(CharacterList, sb);
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Id", "Name", "Index", "CharacterList", "PrefabPath" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Id, r.Name, r.Index, r.CharacterList, r.PrefabPath });
        ...
        public Record GetByIdOrThrow(int key)
        {
            if (!_recordsById.TryGetValue(key, out Record record))
                throw new SheetManException($"There is no record in table `CollectionGroupTable` that corresponds to field `Id` value {key}");
        ...
        public Task ReadAsync(LiteBinaryReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        LiteBinaryTable.CheckColumn(column, "CollectionGroupTable.Id", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                    ...
                    case 2:
                        LiteBinaryTable.CheckColumn(column, "CollectionGroupTable.Name", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementString);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 3:
                        LiteBinaryTable.CheckColumn(column, "CollectionGroupTable.Index", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 4:
                        LiteBinaryTable.CheckColumn(column, "CollectionGroupTable.CharacterList", LiteBinaryTable.KindVarArray, 0, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                            reader.TryReadCounter32(out int elementCount);
                            record._characterList = new int[elementCount];
                            for (int j = 0; j < elementCount; ++j)
                            {
                                record._characterList[j] = reader.ReadI32As(column.Element);
                            }
                        }
                        break;

                    case 5:
                        LiteBinaryTable.CheckColumn(column, "CollectionGroupTable.PrefabPath", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementString);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
