...
{
    [System.Serializable]
    public partial class CostCurveTableTable
    {
        #region Record
        [System.Serializable]
        public partial class Record
        {
            ...
            /// 재화 타입
            /// </summary>
            public global::Rescue.Tables.CurrencyType CostType => _costType;

            /// <summary>
            /// 기본 비용
            /// </summary>
            public int BaseCostValue => _baseCostValue;
            #endregion
            ...
            internal global::Rescue.Tables.CurrencyType _costType;
            internal int _baseCostValue;
            #endregion
            ...
            public override string ToString()
            {
                ...
                sb.Append(",\"CostType\":"); ToStringHelper.ToString(CostType, sb);
                sb.Append(",\"BaseCostValue\":"); ToStringHelper.ToString(BaseCostValue, sb);
                sb.Append("}");
                return sb.ToString();
            }
            #endregion
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Id", "Name", "GrowthType", "CostType", "BaseCostValue" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Id, r.Name, r.GrowthType, r.CostType, r.BaseCostValue });
        ...
        public Record GetByIdOrThrow(int key)
        {
            if (!_recordsById.TryGetValue(key, out Record record))
                throw new SheetManException($"There is no record in table `CostCurveTable` that corresponds to field `Id` value {key}");
        ...
        public Task ReadAsync(LiteBinaryReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        LiteBinaryTable.CheckColumn(column, "CostCurveTable.Id", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                    ...
                    case 2:
                        LiteBinaryTable.CheckColumn(column, "CostCurveTable.Name", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementString);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 3:
                        LiteBinaryTable.CheckColumn(column, "CostCurveTable.GrowthType", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 4:
                        LiteBinaryTable.CheckColumn(column, "CostCurveTable.CostType", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                            reader.ReadOptimalInt32(out tempEnumInt);
                            record._costType = (global::Rescue.Tables.CurrencyType)tempEnumInt;
                        }
                        break;

                    case 5:
                        LiteBinaryTable.CheckColumn(column, "CostCurveTable.BaseCostValue", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
