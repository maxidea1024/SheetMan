...
{
    [System.Serializable]
    public partial class ShortCutTableTable
    {
        #region Record
        [System.Serializable]
        ...
        public Record GetByIDOrThrow(int key)
        {
            if (!_recordsByID.TryGetValue(key, out Record record))
                throw new SheetManException($"There is no record in table `ShortCutTable` that corresponds to field `ID` value {key}");
        ...
        public Task ReadAsync(LiteBinaryReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        LiteBinaryTable.CheckColumn(column, "ShortCutTable.ID", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                    ...
                    case 2:
                        LiteBinaryTable.CheckColumn(column, "ShortCutTable.Name", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementString);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 3:
                        LiteBinaryTable.CheckColumn(column, "ShortCutTable.Type", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                            reader.ReadOptimalInt32(out tempEnumInt);
                            record._type = (global::Rescue.Tables.ShortCutType)tempEnumInt;
                        }
                        break;

                    case 4:
                        LiteBinaryTable.CheckColumn(column, "ShortCutTable.SubIndex", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementI32, LiteBinaryTable.ElementVarint);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                    ...
                    case 5:
                        LiteBinaryTable.CheckColumn(column, "ShortCutTable.Description", LiteBinaryTable.KindScalar, 1, LiteBinaryTable.ElementString);
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
