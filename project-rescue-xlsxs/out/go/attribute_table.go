...
// AttributeRecord was generated from project-rescue-xlsxs/MasterData.xlsx : AttributeTable : A1.
...
// AttributeTable holds every row of Attribute.
...
func (t *AttributeTable) Records() []AttributeRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *AttributeTable) FindById(key int32) *AttributeRecord {
...
func (t *AttributeTable) GetByIdOrError(key int32) (*AttributeRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Attribute` that corresponds to field `Id` value %v", key)
	}
...
func (t *AttributeTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *AttributeTable) Read(filename string) error {
	...
	records := make([]AttributeRecord, count)
