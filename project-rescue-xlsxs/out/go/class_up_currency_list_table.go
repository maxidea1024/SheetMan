...
// ClassUpCurrencyListRecord was generated from project-rescue-xlsxs/ItemData.xlsx : ClassUpCurrencyListTable : A1.
type ClassUpCurrencyListRecord struct {
	// Currency_Table
	Id int32
	// 한글
	Name string
...
// ClassUpCurrencyListTable holds every row of ClassUpCurrencyList.
type ClassUpCurrencyListTable struct {
	records []ClassUpCurrencyListRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *ClassUpCurrencyListTable) Records() []ClassUpCurrencyListRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *ClassUpCurrencyListTable) FindById(key int32) *ClassUpCurrencyListRecord {
...
func (t *ClassUpCurrencyListTable) GetByIdOrError(key int32) (*ClassUpCurrencyListRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `ClassUpCurrencyList` that corresponds to field `Id` value %v", key)
	}
...
func (t *ClassUpCurrencyListTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *ClassUpCurrencyListTable) Read(filename string) error {
	...
	records := make([]ClassUpCurrencyListRecord, count)
	...
	for _, column := range columns {
		...
		case 8:
			if sheetman.CheckColumn(reader, column, "ClassUpCurrencyList.ShortCutIndex", sheetman.KindVarArray, 0, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("ClassUpCurrencyList: %w", err)
	}
