...
// CollectionGroupRecord was generated from project-rescue-xlsxs/CollectionData.xlsx : CollectionGroupTable : A1.
type CollectionGroupRecord struct {
	// ID
	Id int32
	// 영문
	Name string
...
// CollectionGroupTable holds every row of CollectionGroup.
type CollectionGroupTable struct {
	records []CollectionGroupRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CollectionGroupTable) Records() []CollectionGroupRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *CollectionGroupTable) FindById(key int32) *CollectionGroupRecord {
...
func (t *CollectionGroupTable) GetByIdOrError(key int32) (*CollectionGroupRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `CollectionGroup` that corresponds to field `Id` value %v", key)
	}
...
func (t *CollectionGroupTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *CollectionGroupTable) Read(filename string) error {
	...
	records := make([]CollectionGroupRecord, count)
	...
	for _, column := range columns {
		...
		case 4:
			if sheetman.CheckColumn(reader, column, "CollectionGroup.CharacterList", sheetman.KindVarArray, 0, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("CollectionGroup: %w", err)
	}
