...
// CostCurveRangeRecord was generated from project-rescue-xlsxs/StageGrowth.xlsx : CostCurveRangeTable : A1.
type CostCurveRangeRecord struct {
	// 숫자
	Id int32
	// 비용 콘텐츠 ID
...
// CostCurveRangeTable holds every row of CostCurveRange.
type CostCurveRangeTable struct {
	records []CostCurveRangeRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CostCurveRangeTable) Records() []CostCurveRangeRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *CostCurveRangeTable) FindById(key int32) *CostCurveRangeRecord {
...
func (t *CostCurveRangeTable) GetByIdOrError(key int32) (*CostCurveRangeRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `CostCurveRange` that corresponds to field `Id` value %v", key)
	}
...
func (t *CostCurveRangeTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *CostCurveRangeTable) Read(filename string) error {
	...
	records := make([]CostCurveRangeRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("CostCurveRange: %w", err)
	}
