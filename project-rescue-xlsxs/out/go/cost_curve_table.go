...
// CostCurveRecord was generated from project-rescue-xlsxs/StageGrowth.xlsx : CostCurveTable : A1.
type CostCurveRecord struct {
	// 아이디
	Id int32
	// 콘텐츠 타입
	Name string
...
// CostCurveTable holds every row of CostCurve.
type CostCurveTable struct {
	records []CostCurveRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CostCurveTable) Records() []CostCurveRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *CostCurveTable) FindById(key int32) *CostCurveRecord {
...
func (t *CostCurveTable) GetByIdOrError(key int32) (*CostCurveRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `CostCurve` that corresponds to field `Id` value %v", key)
	}
...
func (t *CostCurveTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *CostCurveTable) Read(filename string) error {
	...
	records := make([]CostCurveRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("CostCurve: %w", err)
	}
