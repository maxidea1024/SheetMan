...
// CostCurveTableTable holds every row of CostCurveTable.
type CostCurveTableTable struct {
	records []CostCurveTableRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CostCurveTableTable) Records() []CostCurveTableRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *CostCurveTableTable) FindById(key int32) *CostCurveTableRecord {
...
func (t *CostCurveTableTable) GetByIdOrError(key int32) (*CostCurveTableRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `CostCurveTable` that corresponds to field `Id` value %v", key)
	}
...
func (t *CostCurveTableTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .table file written by SheetMan.
func (t *CostCurveTableTable) Read(filename string) error {
	...
	records := make([]CostCurveTableRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if sheetman.CheckColumn(reader, column, "CostCurveTable.Id", sheetman.KindScalar, 1, sheetman.ElementI32, sheetman.ElementVarint) {
		...
		case 2:
			if sheetman.CheckColumn(reader, column, "CostCurveTable.Name", sheetman.KindScalar, 1, sheetman.ElementString) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
		...
		case 3:
			if sheetman.CheckColumn(reader, column, "CostCurveTable.GrowthType", sheetman.KindScalar, 1, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
		...
		case 4:
			if sheetman.CheckColumn(reader, column, "CostCurveTable.CostType", sheetman.KindScalar, 1, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
		...
		case 5:
			if sheetman.CheckColumn(reader, column, "CostCurveTable.BaseCostValue", sheetman.KindScalar, 1, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("CostCurveTable: %w", err)
	}
