...
// ExpDungeonStageRecord was generated from project-rescue-xlsxs/DailyDungeon.xlsx : ExpDungeonStageTable : A1.
type ExpDungeonStageRecord struct {
...
// ExpDungeonStageTable holds every row of ExpDungeonStage.
type ExpDungeonStageTable struct {
	records []ExpDungeonStageRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *ExpDungeonStageTable) Records() []ExpDungeonStageRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *ExpDungeonStageTable) FindById(key int32) *ExpDungeonStageRecord {
...
func (t *ExpDungeonStageTable) GetByIdOrError(key int32) (*ExpDungeonStageRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `ExpDungeonStage` that corresponds to field `Id` value %v", key)
	}
...
func (t *ExpDungeonStageTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *ExpDungeonStageTable) Read(filename string) error {
	...
	records := make([]ExpDungeonStageRecord, count)
	...
	for _, column := range columns {
		...
		case 7:
			if sheetman.CheckColumn(reader, column, "ExpDungeonStage.SpawnIds", sheetman.KindVarArray, 0, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
		...
		case 8:
			if sheetman.CheckColumn(reader, column, "ExpDungeonStage.BossID", sheetman.KindVarArray, 0, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(reader.ReadCounter32())
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("ExpDungeonStage: %w", err)
	}
