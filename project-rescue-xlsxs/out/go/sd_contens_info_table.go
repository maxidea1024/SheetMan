...
// SDContensInfoRecord was generated from project-rescue-xlsxs/SD_Contens.xlsx : SDContensInfoTable : A1.
type SDContensInfoRecord struct {
	// SDContensInfo_Table
	Id int32
	// 영문
	Name string
...
// SDContensInfoTable holds every row of SDContensInfo.
type SDContensInfoTable struct {
	records []SDContensInfoRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *SDContensInfoTable) Records() []SDContensInfoRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *SDContensInfoTable) FindById(key int32) *SDContensInfoRecord {
...
func (t *SDContensInfoTable) GetByIdOrError(key int32) (*SDContensInfoRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `SDContensInfo` that corresponds to field `Id` value %v", key)
	}
...
func (t *SDContensInfoTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *SDContensInfoTable) Read(filename string) error {
	...
	records := make([]SDContensInfoRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("SDContensInfo: %w", err)
	}
