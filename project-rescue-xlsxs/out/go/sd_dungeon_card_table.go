...
// SDDungeonCardRecord was generated from project-rescue-xlsxs/SD_Contens.xlsx : SDDungeonCardTable : A1.
type SDDungeonCardRecord struct {
	// SDDungeonCard_Table
	Id int32
	// 영문명
	Name string
...
// SDDungeonCardTable holds every row of SDDungeonCard.
type SDDungeonCardTable struct {
	records []SDDungeonCardRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *SDDungeonCardTable) Records() []SDDungeonCardRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *SDDungeonCardTable) FindById(key int32) *SDDungeonCardRecord {
...
func (t *SDDungeonCardTable) GetByIdOrError(key int32) (*SDDungeonCardRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `SDDungeonCard` that corresponds to field `Id` value %v", key)
	}
...
func (t *SDDungeonCardTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *SDDungeonCardTable) Read(filename string) error {
	...
	records := make([]SDDungeonCardRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("SDDungeonCard: %w", err)
	}
