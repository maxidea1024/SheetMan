...
// SDTrainingLevelRecord was generated from project-rescue-xlsxs/SD_Contens.xlsx : SDTrainingLevelTable : A1.
type SDTrainingLevelRecord struct {
...
// SDTrainingLevelTable holds every row of SDTrainingLevel.
type SDTrainingLevelTable struct {
	records []SDTrainingLevelRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *SDTrainingLevelTable) Records() []SDTrainingLevelRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *SDTrainingLevelTable) FindById(key int32) *SDTrainingLevelRecord {
...
func (t *SDTrainingLevelTable) GetByIdOrError(key int32) (*SDTrainingLevelRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `SDTrainingLevel` that corresponds to field `Id` value %v", key)
	}
...
func (t *SDTrainingLevelTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *SDTrainingLevelTable) Read(filename string) error {
	...
	records := make([]SDTrainingLevelRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("SDTrainingLevel: %w", err)
	}
