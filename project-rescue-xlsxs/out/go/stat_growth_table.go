...
// StatGrowthRecord was generated from project-rescue-xlsxs/StageGrowth.xlsx : StatGrowthTable : A1.
type StatGrowthRecord struct {
	// 기본값
	Id int32
	// 영문
	Name string
...
// StatGrowthTable holds every row of StatGrowth.
type StatGrowthTable struct {
	records []StatGrowthRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *StatGrowthTable) Records() []StatGrowthRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *StatGrowthTable) FindById(key int32) *StatGrowthRecord {
...
func (t *StatGrowthTable) GetByIdOrError(key int32) (*StatGrowthRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `StatGrowth` that corresponds to field `Id` value %v", key)
	}
...
func (t *StatGrowthTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *StatGrowthTable) Read(filename string) error {
	...
	records := make([]StatGrowthRecord, count)
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("StatGrowth: %w", err)
	}
