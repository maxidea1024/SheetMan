...
// TraitRecord was generated from project-rescue-xlsxs/StageGrowth.xlsx : TraitTable : A1.
...
// TraitTable holds every row of Trait.
...
func (t *TraitTable) Records() []TraitRecord { return t.records }

// FindById returns the row with this Id, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *TraitTable) FindById(key int32) *TraitRecord {
...
func (t *TraitTable) GetByIdOrError(key int32) (*TraitRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Trait` that corresponds to field `Id` value %v", key)
	}
...
func (t *TraitTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}

// Read loads the table from a .scb file written by SheetMan.
func (t *TraitTable) Read(filename string) error {
	...
	records := make([]TraitRecord, count)
	...
	for _, column := range columns {
		...
		case 6:
			if sheetman.CheckColumn(reader, column, "Trait.UnlockCondition", sheetman.KindVarArray, 0, sheetman.ElementI32, sheetman.ElementVarint) {
				for i := int32(0); i < count; i++ {
					r := &records[i]
