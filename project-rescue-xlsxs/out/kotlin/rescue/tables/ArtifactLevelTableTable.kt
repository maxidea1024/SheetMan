...
class ArtifactLevelTableTable {
     ...
     */
    var records: MutableList<ArtifactLevelTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, ArtifactLevelTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ArtifactLevelTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ArtifactLevelTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ArtifactLevelTableRecord>(count)
        val loadedById = HashMap<Int, ArtifactLevelTableRecord>(count * 2)

        repeat(count) { loaded.add(ArtifactLevelTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ArtifactLevelTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "ArtifactLevelTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "ArtifactLevelTable.NameKR", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.nameKR = reader.readString()
                    }
                }
                4 -> {
                    checkColumn(column, "ArtifactLevelTable.Level", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.level = reader.readI32As(column.element)
                    }
                }
                5 -> {
                    checkColumn(column, "ArtifactLevelTable.CharacterEXP", KIND_SCALAR, 1, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.characterEXP = reader.readI64As(column.element)
                    }
                }
                6 -> {
                    checkColumn(column, "ArtifactLevelTable.AccumulatedEXP", KIND_SCALAR, 1, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.accumulatedEXP = reader.readI64As(column.element)
                    }
                }
                7 -> {
                    checkColumn(column, "ArtifactLevelTable.ATKGrowth", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
                        record.aTKGrowth = reader.readFloat()
                    }
                }
                8 -> {
                    checkColumn(column, "ArtifactLevelTable.DEFGrowth", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
                        record.dEFGrowth = reader.readFloat()
                    }
                }
                9 -> {
                    checkColumn(column, "ArtifactLevelTable.HPGrowth", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
