...
class AttributeTableTable {
     ...
     */
    var records: MutableList<AttributeTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, AttributeTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): AttributeTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): AttributeTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<AttributeTableRecord>(count)
        val loadedById = HashMap<Int, AttributeTableRecord>(count * 2)

        repeat(count) { loaded.add(AttributeTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "AttributeTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "AttributeTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "AttributeTable.AttributeName", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.attributeName = reader.readString()
                    }
                }
                4 -> {
                    checkColumn(column, "AttributeTable.AttributeType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.attributeType = AttributeType.of(reader.readEnum())
                    }
                }
                5 -> {
                    checkColumn(column, "AttributeTable.TargetAttributeType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.targetAttributeType = AttributeType.of(reader.readEnum())
                    }
                }
                6 -> {
                    checkColumn(column, "AttributeTable.DamageValueRate", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
                        record.damageValueRate = reader.readFloat()
                    }
                }
                7 -> {
                    checkColumn(column, "AttributeTable.IsVisible", KIND_SCALAR, 1, ELEMENT_BOOL)
                    for (record in loaded) {
                        record.isVisible = reader.readBool()
                    }
                }
                8 -> {
                    checkColumn(column, "AttributeTable.IconPath", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
