...
class BGMSoundTableTable {
     ...
     */
    var records: MutableList<BGMSoundTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, BGMSoundTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): BGMSoundTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): BGMSoundTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<BGMSoundTableRecord>(count)
        val loadedById = HashMap<Int, BGMSoundTableRecord>(count * 2)

        repeat(count) { loaded.add(BGMSoundTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "BGMSoundTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "BGMSoundTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "BGMSoundTable.Path", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.path = reader.readString()
                    }
                }
                4 -> {
                    checkColumn(column, "BGMSoundTable.Loop", KIND_SCALAR, 1, ELEMENT_BOOL)
                    for (record in loaded) {
                        record.loop = reader.readBool()
                    }
                }
                5 -> {
                    checkColumn(column, "BGMSoundTable.FadeTime", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
                        record.fadeTime = reader.readFloat()
                    }
                }
                6 -> {
                    checkColumn(column, "BGMSoundTable.Description", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
