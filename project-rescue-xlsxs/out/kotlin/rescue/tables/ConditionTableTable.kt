...
class ConditionTableTable {
     ...
     */
    var records: MutableList<ConditionTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, ConditionTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ConditionTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ConditionTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ConditionTableRecord>(count)
        val loadedById = HashMap<Int, ConditionTableRecord>(count * 2)

        repeat(count) { loaded.add(ConditionTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ConditionTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "ConditionTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "ConditionTable.ConditionTargetType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.conditionTargetType = ConditionTargetType.of(reader.readEnum())
                    }
                }
                4 -> {
                    checkColumn(column, "ConditionTable.ConditionTargetValue", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.conditionTargetValue = reader.readI32As(column.element)
                    }
                }
                5 -> {
                    checkColumn(column, "ConditionTable.ConditionType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.conditionType = ConditionType.of(reader.readEnum())
                    }
                }
                6 -> {
                    checkColumn(column, "ConditionTable.ConditionValue", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
