...
class CostCurveTableRecord {
    ...
    /** 재화 타입 */
    var costType: CurrencyType = CurrencyType.of(0)
    /** 기본 비용 */
    var baseCostValue: Int = 0
...
class CostCurveTableTable {
     ...
     */
    var records: MutableList<CostCurveTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, CostCurveTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): CostCurveTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): CostCurveTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<CostCurveTableRecord>(count)
        val loadedById = HashMap<Int, CostCurveTableRecord>(count * 2)

        repeat(count) { loaded.add(CostCurveTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "CostCurveTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "CostCurveTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "CostCurveTable.GrowthType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.growthType = GrowthType.of(reader.readEnum())
                    }
                }
                4 -> {
                    checkColumn(column, "CostCurveTable.CostType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.costType = CurrencyType.of(reader.readEnum())
                    }
                }
                5 -> {
                    checkColumn(column, "CostCurveTable.BaseCostValue", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
