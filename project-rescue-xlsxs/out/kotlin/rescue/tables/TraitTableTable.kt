...
class TraitTableTable {
     ...
     */
    var records: MutableList<TraitTableRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, TraitTableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): TraitTableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): TraitTableRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<TraitTableRecord>(count)
        val loadedById = HashMap<Int, TraitTableRecord>(count * 2)

        repeat(count) { loaded.add(TraitTableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "TraitTable.Id", KIND_SCALAR, 1, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.id = reader.readI32As(column.element)
                    }
                }
                2 -> {
                    checkColumn(column, "TraitTable.Name", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.name = reader.readString()
                    }
                }
                3 -> {
                    checkColumn(column, "TraitTable.TraitName", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
                        record.traitName = reader.readString()
                    }
                }
                4 -> {
                    checkColumn(column, "TraitTable.StatType", KIND_SCALAR, 1, ELEMENT_VARINT)
                    for (record in loaded) {
                        record.statType = StatType.of(reader.readEnum())
                    }
                }
                5 -> {
                    checkColumn(column, "TraitTable.StatRate", KIND_SCALAR, 1, ELEMENT_F32)
                    for (record in loaded) {
                        record.statRate = reader.readFloat()
                    }
                }
                6 -> {
                    checkColumn(column, "TraitTable.UnlockCondition", KIND_VAR_ARRAY, 0, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
                ...
                7 -> {
                    checkColumn(column, "TraitTable.IconPath", KIND_SCALAR, 1, ELEMENT_STRING)
                    for (record in loaded) {
