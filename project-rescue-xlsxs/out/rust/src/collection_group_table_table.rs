...
impl CollectionGroupTableRecord {
}

/// Every row of CollectionGroupTable.
#[derive(Clone, Debug, Default)]
pub struct CollectionGroupTableTable {
    records: Vec<CollectionGroupTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl CollectionGroupTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[CollectionGroupTableRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&CollectionGroupTableRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&CollectionGroupTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "CollectionGroupTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![CollectionGroupTableRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    sheetman::check_column(column, "CollectionGroupTable.Id", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.id = reader.read_i32_as(column.element)?;
                    }
                }
                2 => {
                    sheetman::check_column(column, "CollectionGroupTable.Name", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.name = reader.read_string()?;
                    }
                }
                3 => {
                    sheetman::check_column(column, "CollectionGroupTable.Index", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.index = reader.read_i32_as(column.element)?;
                    }
                }
                4 => {
                    sheetman::check_column(column, "CollectionGroupTable.CharacterList", sheetman::KIND_VAR_ARRAY, 0, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                ...
                5 => {
                    sheetman::check_column(column, "CollectionGroupTable.PrefabPath", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
