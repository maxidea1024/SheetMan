...
impl InfoGrowthTableRecord {
}

/// Every row of InfoGrowthTable.
#[derive(Clone, Debug, Default)]
pub struct InfoGrowthTableTable {
    records: Vec<InfoGrowthTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl InfoGrowthTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[InfoGrowthTableRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&InfoGrowthTableRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&InfoGrowthTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "InfoGrowthTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![InfoGrowthTableRecord::default(); count];
