...
pub struct SDAlchemyStepTableRecord {
    /// SDAlchemyStep_Table
    pub id: i32,
    /// 영문명
    pub name: String,
...
impl SDAlchemyStepTableRecord {
}

/// Every row of SDAlchemyStepTable.
#[derive(Clone, Debug, Default)]
pub struct SDAlchemyStepTableTable {
    records: Vec<SDAlchemyStepTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl SDAlchemyStepTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[SDAlchemyStepTableRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&SDAlchemyStepTableRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&SDAlchemyStepTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "SDAlchemyStepTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![SDAlchemyStepTableRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.Id", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.id = reader.read_i32_as(column.element)?;
                    }
                }
                2 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.Name", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.name = reader.read_string()?;
                    }
                }
                3 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.NameKR", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.name_kr = reader.read_string()?;
                    }
                }
                4 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.MaxLevel", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.max_level = reader.read_i32_as(column.element)?;
                    }
                }
                5 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.RewardType", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.reward_type = CurrencyType::from_value(reader.read_enum()?).unwrap_or_default();
                    }
                }
                6 => {
                    sheetman::check_column(column, "SDAlchemyStepTable.RewardValue", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
