// ------------------------------------------------------------------------------
// Generated by SheetMan. DO NOT EDIT.
//
// Changes to this file may cause incorrect behavior and will be lost if the code is
// regenerated.
// ------------------------------------------------------------------------------

use std::collections::HashMap;
use std::path::Path;
use crate::sheetman;
use crate::enum_sd_contens_type::SdContensType;
use crate::enum_currency_type::CurrencyType;
use crate::enum_cycle_type::CycleType;

// Generated from project-rescue-xlsxs/SD_Contens.xlsx : SDContensInfoTable : A1
#[derive(Clone, Debug, Default)]
pub struct SDContensInfoTableRecord {
    /// SDContensInfo_Table
    pub id: i32,
    /// 영문
    pub name: String,
    /// 한글
    pub info_name: String,
    /// 실제 데이터가 있는 시트 이름
    pub sheet_name: String,
    /// SD 콘텐츠 타입
    pub sd_contens_type: SdContensType,
    /// 콘텐츠 해금 조건 ID
    pub condition_id: i32,
    /// 소모 재화
    pub currency_type: CurrencyType,
    /// 초기화 적용 유무
    pub enable_reset: bool,
    /// 초기화 타입
    pub cycle_type: CycleType,
    /// SD  캐릭터 프리펩 경로
    pub sd_character_path: String,
    /// UI 3d프리펩 경로
    pub prefab_path: String,
}

impl SDContensInfoTableRecord {
}

/// Every row of SDContensInfoTable.
#[derive(Clone, Debug, Default)]
pub struct SDContensInfoTableTable {
    records: Vec<SDContensInfoTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl SDContensInfoTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[SDContensInfoTableRecord] {
        &self.records
    }

    /// The row with this Id, or None when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    pub fn find_by_id(&self, key: i32) -> Option<&SDContensInfoTableRecord> {
        self.by_id.get(&key).map(|position| &self.records[*position])
    }

    /// The row with this Id, or an error naming what was missing.
    ///
    /// For a key that has to be there - one from another table, or a constant. Rust has no
    /// exception to throw, so where the other languages name a throw this one names the
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&SDContensInfoTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "SDContensInfoTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }

    /// Whether the table holds a row with this Id.
    pub fn contains_id(&self, key: i32) -> bool {
        self.by_id.contains_key(&key)
    }

    /// Loads the table from a .scb file written by SheetMan.
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        let data = sheetman::read_all_bytes(filename)?;
        let mut reader = sheetman::Reader::new(&data);

        // Column by column, matched by tag rather than position: a column this build
        // does not know is skipped by its block length, and one whose type changed
        // incompatibly fails naming the field.
        let header = sheetman::read_table_header(&mut reader)?;
        let count = header.row_count.max(0) as usize;

        // Read into rows of its own and assigned at the end. Reading a table that is
        // already loaded is a refresh, and one that turns out to be unreadable has to
        // leave the rows already there alone - `?` returns before anything is replaced.
        let mut records = vec![SDContensInfoTableRecord::default(); count];

        for column in &header.columns {
            let block_end = reader.position() + column.byte_length.max(0) as usize;

            match column.tag {
                1 => {
                    sheetman::check_column(column, "SDContensInfoTable.Id", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.id = reader.read_i32_as(column.element)?;
                    }
                }
                2 => {
                    sheetman::check_column(column, "SDContensInfoTable.Name", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.name = reader.read_string()?;
                    }
                }
                3 => {
                    sheetman::check_column(column, "SDContensInfoTable.InfoName", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.info_name = reader.read_string()?;
                    }
                }
                4 => {
                    sheetman::check_column(column, "SDContensInfoTable.SheetName", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.sheet_name = reader.read_string()?;
                    }
                }
                5 => {
                    sheetman::check_column(column, "SDContensInfoTable.SdContensType", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.sd_contens_type = SdContensType::from_value(reader.read_enum()?).unwrap_or_default();
                    }
                }
                6 => {
                    sheetman::check_column(column, "SDContensInfoTable.ConditionID", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_I32, sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.condition_id = reader.read_i32_as(column.element)?;
                    }
                }
                7 => {
                    sheetman::check_column(column, "SDContensInfoTable.CurrencyType", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.currency_type = CurrencyType::from_value(reader.read_enum()?).unwrap_or_default();
                    }
                }
                8 => {
                    sheetman::check_column(column, "SDContensInfoTable.EnableReset", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_BOOL])?;
                    for record in records.iter_mut() {
                        record.enable_reset = reader.read_bool()?;
                    }
                }
                9 => {
                    sheetman::check_column(column, "SDContensInfoTable.CycleType", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_VARINT])?;
                    for record in records.iter_mut() {
                        record.cycle_type = CycleType::from_value(reader.read_enum()?).unwrap_or_default();
                    }
                }
                10 => {
                    sheetman::check_column(column, "SDContensInfoTable.SDCharacterPath", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.sd_character_path = reader.read_string()?;
                    }
                }
                11 => {
                    sheetman::check_column(column, "SDContensInfoTable.PrefabPath", sheetman::KIND_SCALAR, 1, &[sheetman::ELEMENT_STRING])?;
                    for record in records.iter_mut() {
                        record.prefab_path = reader.read_string()?;
                    }
                }
                _ => {
                    // A column added after this code was generated.
                    reader.skip(column.byte_length)?;
                }
            }

            sheetman::check_block_end(&reader, column, block_end)?;
        }

        let by_id = records
            .iter()
            .enumerate()
            .map(|(position, record)| (record.id.clone(), position))
            .collect();

        // Published, now that every column read.
        self.records = records;
        self.by_id = by_id;

        Ok(())
    }
}
