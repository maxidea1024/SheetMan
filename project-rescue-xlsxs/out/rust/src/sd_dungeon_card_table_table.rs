...
impl SDDungeonCardTableRecord {
}

/// Every row of SDDungeonCardTable.
#[derive(Clone, Debug, Default)]
pub struct SDDungeonCardTableTable {
    records: Vec<SDDungeonCardTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl SDDungeonCardTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[SDDungeonCardTableRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&SDDungeonCardTableRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&SDDungeonCardTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "SDDungeonCardTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![SDDungeonCardTableRecord::default(); count];
