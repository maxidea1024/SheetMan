...
pub struct SDPubInfoTableRecord {
    /// SDPubInfo_Table
    pub id: i32,
    /// 영문명
    pub name: String,
...
impl SDPubInfoTableRecord {
}

/// Every row of SDPubInfoTable.
#[derive(Clone, Debug, Default)]
pub struct SDPubInfoTableTable {
    records: Vec<SDPubInfoTableRecord>,
    by_id: HashMap<i32, usize>,
}

impl SDPubInfoTableTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[SDPubInfoTableRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&SDPubInfoTableRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&SDPubInfoTableRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "SDPubInfoTable",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![SDPubInfoTableRecord::default(); count];
