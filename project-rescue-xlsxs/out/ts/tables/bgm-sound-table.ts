...
export class BGMSoundTableTable {
  ...
  /** All records. */
  public get records(): BGMSoundTableRecord[] { return this._records }
  private _records: BGMSoundTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, BGMSoundTableRecord> { return this._recordsById }
  private _recordsById: Map<number, BGMSoundTableRecord> = new Map<number, BGMSoundTableRecord>()
   ...
   */
  public findById(key: number): BGMSoundTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): BGMSoundTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "BGMSoundTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: BGMSoundTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new BGMSoundTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new BGMSoundTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'BGMSoundTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'BGMSoundTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'BGMSoundTable.Path', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'BGMSoundTable.Loop', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_BOOL])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'BGMSoundTable.FadeTime', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 6:
          sheetman.checkColumn(column, 'BGMSoundTable.Description', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: BGMSoundTableRecord[]): void {
    const recordsById = new Map<number, BGMSoundTableRecord>()
