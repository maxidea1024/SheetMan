...
export class CollectionGroupTableTable {
  ...
  /** All records. */
  public get records(): CollectionGroupTableRecord[] { return this._records }
  private _records: CollectionGroupTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, CollectionGroupTableRecord> { return this._recordsById }
  private _recordsById: Map<number, CollectionGroupTableRecord> = new Map<number, CollectionGroupTableRecord>()
   ...
   */
  public findById(key: number): CollectionGroupTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CollectionGroupTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "CollectionGroupTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: CollectionGroupTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CollectionGroupTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CollectionGroupTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'CollectionGroupTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'CollectionGroupTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'CollectionGroupTable.Index', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'CollectionGroupTable.CharacterList', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'CollectionGroupTable.PrefabPath', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: CollectionGroupTableRecord[]): void {
    const recordsById = new Map<number, CollectionGroupTableRecord>()
