...
export class ConfigTableTable {
  ...
  /** All records. */
  public get records(): ConfigTableRecord[] { return this._records }
  private _records: ConfigTableRecord[] = []

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, ConfigTableRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, ConfigTableRecord> = new Map<number, ConfigTableRecord>()
   ...
   */
  public findByIndex(key: number): ConfigTableRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): ConfigTableRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "ConfigTable" that corresponds to field "index" value ${key}`)
  ...
  // Indexing by 'id'
  public get recordsById(): Map<string, ConfigTableRecord> { return this._recordsById }
  private _recordsById: Map<string, ConfigTableRecord> = new Map<string, ConfigTableRecord>()
   ...
   * The lookup to reach for when a missing row is an ordinary answer.
   */
  public findById(key: string): ConfigTableRecord | undefined {
   ...
   * next line is a check or a catch.
   */
  public getByIdOrThrow(key: string): ConfigTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "ConfigTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: ConfigTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new ConfigTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new ConfigTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'ConfigTable.Index', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'ConfigTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._id = reader.readString()
          }
          break
        case 3:
          sheetman.checkColumn(column, 'ConfigTable.Category', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._category = reader.readString()
          }
          break
        case 4:
          sheetman.checkColumn(column, 'ConfigTable.DataType', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._dataType = reader.readString()
          }
          break
        case 5:
          sheetman.checkColumn(column, 'ConfigTable.DefaultValue', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._defaultValue = reader.readString()
          }
          break
        case 6:
          sheetman.checkColumn(column, 'ConfigTable.Description', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: ConfigTableRecord[]): void {
    const recordsByIndex = new Map<number, ConfigTableRecord>()
    const recordsById = new Map<string, ConfigTableRecord>()
