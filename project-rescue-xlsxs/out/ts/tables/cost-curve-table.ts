...
export class CostCurveTableTable {
  ...
  /** All records. */
  public get records(): CostCurveTableRecord[] { return this._records }
  private _records: CostCurveTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, CostCurveTableRecord> { return this._recordsById }
  private _recordsById: Map<number, CostCurveTableRecord> = new Map<number, CostCurveTableRecord>()
   ...
   */
  public findById(key: number): CostCurveTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CostCurveTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "CostCurveTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: CostCurveTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CostCurveTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CostCurveTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'CostCurveTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'CostCurveTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'CostCurveTable.GrowthType', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'CostCurveTable.CostType', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._costType = reader.readEnum() as CurrencyType
          }
          break
        case 5:
          sheetman.checkColumn(column, 'CostCurveTable.BaseCostValue', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._baseCostValue = reader.readI32As(column.element)
          }
          break
        default:
   ...
   */
  private publish(records: CostCurveTableRecord[]): void {
    const recordsById = new Map<number, CostCurveTableRecord>()
