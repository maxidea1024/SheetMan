...
export class SDAlchemyStepTableTable {
  ...
  /** All records. */
  public get records(): SDAlchemyStepTableRecord[] { return this._records }
  private _records: SDAlchemyStepTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, SDAlchemyStepTableRecord> { return this._recordsById }
  private _recordsById: Map<number, SDAlchemyStepTableRecord> = new Map<number, SDAlchemyStepTableRecord>()
   ...
   */
  public findById(key: number): SDAlchemyStepTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): SDAlchemyStepTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "SDAlchemyStepTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: SDAlchemyStepTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new SDAlchemyStepTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new SDAlchemyStepTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.NameKR', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.MaxLevel', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.RewardType', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 6:
          sheetman.checkColumn(column, 'SDAlchemyStepTable.RewardValue', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: SDAlchemyStepTableRecord[]): void {
    const recordsById = new Map<number, SDAlchemyStepTableRecord>()
