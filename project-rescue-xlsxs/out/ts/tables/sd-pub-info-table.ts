...
export class SDPubInfoTableTable {
  ...
  /** All records. */
  public get records(): SDPubInfoTableRecord[] { return this._records }
  private _records: SDPubInfoTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, SDPubInfoTableRecord> { return this._recordsById }
  private _recordsById: Map<number, SDPubInfoTableRecord> = new Map<number, SDPubInfoTableRecord>()
   ...
   */
  public findById(key: number): SDPubInfoTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): SDPubInfoTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "SDPubInfoTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: SDPubInfoTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new SDPubInfoTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new SDPubInfoTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'SDPubInfoTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'SDPubInfoTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'SDPubInfoTable.PubName', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'SDPubInfoTable.UnlockCondition', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'SDPubInfoTable.LimitValue', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 6:
          sheetman.checkColumn(column, 'SDPubInfoTable.BuffID', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 7:
          sheetman.checkColumn(column, 'SDPubInfoTable.Description', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: SDPubInfoTableRecord[]): void {
    const recordsById = new Map<number, SDPubInfoTableRecord>()
