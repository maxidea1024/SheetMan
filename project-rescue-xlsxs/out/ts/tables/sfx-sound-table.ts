...
export class SFXSoundTableTable {
  ...
  /** All records. */
  public get records(): SFXSoundTableRecord[] { return this._records }
  private _records: SFXSoundTableRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, SFXSoundTableRecord> { return this._recordsById }
  private _recordsById: Map<number, SFXSoundTableRecord> = new Map<number, SFXSoundTableRecord>()
   ...
   */
  public findById(key: number): SFXSoundTableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): SFXSoundTableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "SFXSoundTable" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: SFXSoundTableRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new SFXSoundTableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new SFXSoundTableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'SFXSoundTable.Id', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'SFXSoundTable.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'SFXSoundTable.Category', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._category = reader.readEnum() as SFXCategoryType
          }
          break
        case 4:
          sheetman.checkColumn(column, 'SFXSoundTable.Path', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'SFXSoundTable.PreloadGroup', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._preloadGroup = reader.readString()
          }
          break
        case 6:
          sheetman.checkColumn(column, 'SFXSoundTable.Description', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: SFXSoundTableRecord[]): void {
    const recordsById = new Map<number, SFXSoundTableRecord>()
