...
> 이 표는 추측이 아닙니다. `reserved-words` 픽스처가 예약어로 이름 지은 필드를 담고 있고, 회귀 스위트가 그 산출물을 **열 개 언어 전부에서 실제로 컴파일**합니다.
>
...
리더는 각 언어마다 **별도 구현**입니다. 포맷을 정의하는 건 익스포터의 writer 하나이고, 열 개의 리더는 그 하나를 각자 구현한 것이라 어긋날 수 있습니다. 그래서 회귀 스위트가 **C#으로 쓰고 열 개 언어로 각각 읽어 대조**합니다 — 실제로 이 방식이 `long`을 32비트로 잘라내던 writer 버그를 찾아냈습니다.
...
포맷을 **정의하는 코드는 하나**, 그것을 **구현하는 리더는 열한 개**입니다.

|위치|역할|
|--|--|
|`src/Exporters/LiteBinaryWriter.cs`|**포맷을 정의하는 writer.** 익스포터 내부에 있고 외부 의존이 없습니다.|
|`lib/<언어>/sheetman/...`|언어별 리더. `cs` `cpp` `ts` `go` `rust` `python` `java` `kotlin` `ruby` `dart` `unreal` 열한 개|

리더가 `lib/` 아래에 실제 파일로 존재하는 이유는 편집과 리뷰가 가능해야 하기 때문이고, 임베디드 리소스로 읽어 쓰는 이유는 배포본과 커밋된 소스가 어긋날 수 없게 하기 위함입니다.

열한 개가 하나의 정의를 각자 구현하므로 어긋날 수 있고, 어긋나면 **실패하는 게 아니라 값이 달라집니다**. 그래서 적합성 코퍼스가 있습니다 — 경계값 한 테이블을 전 언어로 읽어 익스포터 JSON과 대조합니다. 실제로 이 방식이 Go의 틱 오버플로, Java의 부호 없는 시프트, Dart의 웹 32비트 비트연산, 여러 언어의 비ASCII 인코딩 문제를 출시 전에 잡았습니다.
...
|Unreal|생성된 헤더를 **실제 UnrealHeaderTool**에 통과시킵니다 (`SHEETMAN_UE_ROOT` 지정 시). 엔진 없이도 모듈에 `std::`·표준 헤더·`throw`가 없는지, 손상된 테이블을 거부하는지 확인합니다.|
...
**전 언어 산출물 비교.** [OUT/](OUT/)에 `reserved-words` 픽스처를 **열한 개 생성기 전부로** 만든 결과가 커밋되어 있습니다. 게이트가 아니라 읽기 위한 것입니다 — 스위트는 전부 컴파일해 주지만, 열한 개가 무엇을 뱉는지 나란히 놓고 보여주지는 못합니다. 그 차이가 실제로 문제를 잡습니다: Unreal 타깃이 일반 C++ 리더를 싣고 있던 것도, Dart가 `int int = 0;`을 내던 것도 여기서 드러났습니다.
