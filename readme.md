...
**나오는 것이 다르니 배포도 다릅니다.** enum과 상수셋은 코드로 나가므로 고치면 코드 배포가 함께 필요하고, 테이블은 데이터 파일로 나가므로 대개 데이터만 올려도 됩니다 — [어느 변경이 어느 쪽인지](doc/languages/readme.md#데이터만-나가도-되는-변경과-코드가-함께-나가야-하는-변경).
