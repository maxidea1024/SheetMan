...
위 마커 방식이 기본(`sheetman` 레이아웃)이지만, **이미 다른 규칙으로 만들어져 돌아가는 엑셀도 그대로 읽을 수 있습니다.** 워크북 수백 장을 먼저 고쳐 쓰지 않아도 됩니다.
...
레이아웃은 **소스 항목마다** 지정하므로 한 recipe에서 섞어 읽을 수 있고, 한쪽에서 선언한 enum을 다른 쪽 테이블이 타입으로 써도 됩니다 — 옮기는 도중에 쓰라고 그렇게 만들었습니다.

- 레이아웃별 시트 규칙: [시트 작성 — 다른 레이아웃 읽기](doc/sheets.md#다른-레이아웃-읽기--rescue)
- recipe 설정: [Recipe 파일 — Layout](doc/recipe.md#layout--시트를-읽는-방식)
- **실제 프로젝트를 통째로 읽어본 기록**: [기존 프로젝트 읽어오기](doc/adopting-a-project.md)
...
|[**기존 프로젝트 읽어오기**](doc/adopting-a-project.md)|다른 규칙으로 만들어져 이미 돌아가는 엑셀을 그대로 읽기. 실제 프로젝트 17개 워크북을 변환한 기록|
...
- 변환하면서 걸러낼 수 있는 실수는 최대한 걸러냅니다. 게임에서 터진 뒤에 찾는 대신 변환할 때 알게 됩니다.
...
리더는 언어마다 별도 구현이라 어긋날 수 있습니다. 포맷을 정의하는 건 익스포터의 writer 하나이고, 13개의 리더는 그 하나를 각자 구현한 것입니다 — 그래서 회귀 스위트가 **C#으로 쓰고 12개 언어로 각각 읽어 대조**합니다. 실제로 이 방식이 `long`을 32비트로 잘라내던 writer 버그를 찾아냈습니다.
