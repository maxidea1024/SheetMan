...
#include <string.h>
static bool Rescue_BuffSelectParse(Rescue_BuffSelectTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_BuffSelectRecord_t*)sm_arena_alloc(
  ...
  for (at = 0; at < column_count && !sm_failed(reader); ++at) {
    ...
    case 5:
      (void)sm_check_column(reader, column, "BuffSelect.BuffRate", SM_KIND_SCALAR, 1, SM_ELEMENT_MASK(SM_ELEMENT_F32));

      (void)sm_cursor_init(&cursor, reader, column, table->count, "BuffSelect.BuffRate");

      for (row = 0; row < table->count && !sm_failed(reader); ++row) {
        Rescue_BuffSelectRecord_t* record = &table->records[row];

        (void)sm_cursor_next_f32(&cursor, &record->buff_rate);
      }
...
bool Rescue_BuffSelectLoad(Rescue_BuffSelectTable_t* table, const char* filename,
  ...
  Rescue_BuffSelectTable_t loaded;
