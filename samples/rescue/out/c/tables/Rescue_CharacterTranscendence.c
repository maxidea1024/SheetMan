...
#include <string.h>
static bool Rescue_CharacterTranscendenceParse(Rescue_CharacterTranscendenceTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_CharacterTranscendenceRecord_t*)sm_arena_alloc(
...
bool Rescue_CharacterTranscendenceLoad(Rescue_CharacterTranscendenceTable_t* table, const char* filename,
  ...
  Rescue_CharacterTranscendenceTable_t loaded;
...
void Rescue_CharacterTranscendenceFree(Rescue_CharacterTranscendenceTable_t* table) {
  sm_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const Rescue_CharacterTranscendenceRecord_t* Rescue_CharacterTranscendenceFindById(
  const Rescue_CharacterTranscendenceTable_t* table, int32_t key) {
  int32_t position = sm_index_find(table->by_id, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool Rescue_CharacterTranscendenceContainsId(
  const Rescue_CharacterTranscendenceTable_t* table, int32_t key) {
