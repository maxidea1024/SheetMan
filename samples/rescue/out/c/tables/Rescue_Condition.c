...
#include <string.h>
static bool Rescue_ConditionParse(Rescue_ConditionTable_t* table, sm_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Rescue_ConditionRecord_t*)sm_arena_alloc(
...
bool Rescue_ConditionLoad(Rescue_ConditionTable_t* table, const char* filename,
  ...
  Rescue_ConditionTable_t loaded;
