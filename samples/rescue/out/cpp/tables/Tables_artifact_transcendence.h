...
#ifndef SHEETMAN_GENERATED_TABLES_ARTIFACT_TRANSCENDENCE_H
#define SHEETMAN_GENERATED_TABLES_ARTIFACT_TRANSCENDENCE_H
...
class ArtifactTranscendenceTable {
 public:
  const std::vector<ArtifactTranscendenceRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const ArtifactTranscendenceRecord* find_by_id(std::int32_t key) const {
  ...
  const ArtifactTranscendenceRecord& get_by_id_or_throw(std::int32_t key) const {
    const ArtifactTranscendenceRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `ArtifactTranscendence` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<ArtifactTranscendenceRecord> records;
 ...
 private:
  ...
  void publish(std::vector<ArtifactTranscendenceRecord>&& records) {
  ...
  std::vector<ArtifactTranscendenceRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_ARTIFACT_TRANSCENDENCE_H
