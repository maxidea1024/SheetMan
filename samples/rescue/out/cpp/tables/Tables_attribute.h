...
#ifndef SHEETMAN_GENERATED_TABLES_ATTRIBUTE_H
#define SHEETMAN_GENERATED_TABLES_ATTRIBUTE_H
...
class AttributeTable {
 public:
  const std::vector<AttributeRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const AttributeRecord* find_by_id(std::int32_t key) const {
  ...
  const AttributeRecord& get_by_id_or_throw(std::int32_t key) const {
    const AttributeRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `Attribute` that corresponds to "
 ...
 private:
  ...
  void publish(std::vector<AttributeRecord>&& records) {
  ...
  std::vector<AttributeRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_ATTRIBUTE_H
