...
#ifndef SHEETMAN_GENERATED_TABLES_BGM_SOUND_H
#define SHEETMAN_GENERATED_TABLES_BGM_SOUND_H
...
class BGMSoundTable {
 public:
  const std::vector<BGMSoundRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const BGMSoundRecord* find_by_id(std::int32_t key) const {
  ...
  const BGMSoundRecord& get_by_id_or_throw(std::int32_t key) const {
    const BGMSoundRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `BGMSound` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<BGMSoundRecord> records;
    ...
    for (const sheetman::Column& column : header.columns) {
      ...
      switch (column.tag) {
        case 1: {
          sheetman::check_column(column, "BGMSound.Id", sheetman::kKindScalar, 1, {sheetman::kElementI32, sheetman::kElementVarint});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "BGMSound.Id");
        ...
        case 2: {
          sheetman::check_column(column, "BGMSound.Name", sheetman::kKindScalar, 1, {sheetman::kElementString});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "BGMSound.Name");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
        ...
        case 3: {
          sheetman::check_column(column, "BGMSound.Path", sheetman::kKindScalar, 1, {sheetman::kElementString});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "BGMSound.Path");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
        ...
        case 6: {
          sheetman::check_column(column, "BGMSound.Description", sheetman::kKindScalar, 1, {sheetman::kElementString});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "BGMSound.Description");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
 ...
 private:
  ...
  void publish(std::vector<BGMSoundRecord>&& records) {
  ...
  std::vector<BGMSoundRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_BGM_SOUND_H
