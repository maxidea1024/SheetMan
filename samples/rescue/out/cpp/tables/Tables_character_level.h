...
#ifndef SHEETMAN_GENERATED_TABLES_CHARACTER_LEVEL_H
#define SHEETMAN_GENERATED_TABLES_CHARACTER_LEVEL_H
...
class CharacterLevelTable {
 public:
  const std::vector<CharacterLevelRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const CharacterLevelRecord* find_by_id(std::int32_t key) const {
  ...
  const CharacterLevelRecord& get_by_id_or_throw(std::int32_t key) const {
    const CharacterLevelRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `CharacterLevel` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<CharacterLevelRecord> records;
    ...
    for (const sheetman::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 7: {
          sheetman::check_column(column, "CharacterLevel.ATKGrowth", sheetman::kKindScalar, 1, {sheetman::kElementF32});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "CharacterLevel.ATKGrowth");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
        ...
        case 8: {
          sheetman::check_column(column, "CharacterLevel.DEFGrowth", sheetman::kKindScalar, 1, {sheetman::kElementF32});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "CharacterLevel.DEFGrowth");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
        ...
        case 9: {
          sheetman::check_column(column, "CharacterLevel.HPGrowth", sheetman::kKindScalar, 1, {sheetman::kElementF32});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "CharacterLevel.HPGrowth");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
 ...
 private:
  ...
  void publish(std::vector<CharacterLevelRecord>&& records) {
  ...
  std::vector<CharacterLevelRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_CHARACTER_LEVEL_H
