...
#ifndef SHEETMAN_GENERATED_TABLES_GACHA_RATE_H
#define SHEETMAN_GENERATED_TABLES_GACHA_RATE_H
...
class GachaRateTable {
 public:
  const std::vector<GachaRateRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const GachaRateRecord* find_by_id(std::int32_t key) const {
  ...
  const GachaRateRecord& get_by_id_or_throw(std::int32_t key) const {
    const GachaRateRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `GachaRate` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<GachaRateRecord> records;
    ...
    for (const sheetman::Column& column : header.columns) {
      ...
      switch (column.tag) {
        case 1: {
          sheetman::check_column(column, "GachaRate.Id", sheetman::kKindScalar, 1, {sheetman::kElementI32, sheetman::kElementVarint});
          sheetman::ScbColumnCursor cursor(reader, column, header.row_count, "GachaRate.Id");
 ...
 private:
  ...
  void publish(std::vector<GachaRateRecord>&& records) {
  ...
  std::vector<GachaRateRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_GACHA_RATE_H
