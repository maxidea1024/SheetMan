...
#ifndef SHEETMAN_GENERATED_TABLES_MATERIAL_H
#define SHEETMAN_GENERATED_TABLES_MATERIAL_H
...
class MaterialTable {
 public:
  ...
  const MaterialRecord* find_by_id(std::int32_t key) const {
  ...
  const MaterialRecord& get_by_id_or_throw(std::int32_t key) const {
    const MaterialRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `Material` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    for (const sheetman::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 13: {
          sheetman::check_column(column, "Material.ShortCutIndex", sheetman::kKindVarArray, 0, {sheetman::kElementI32, sheetman::kElementVarint});
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
 ...
 private:
  ...
  void publish(std::vector<MaterialRecord>&& records) {
...
#endif  // SHEETMAN_GENERATED_TABLES_MATERIAL_H
