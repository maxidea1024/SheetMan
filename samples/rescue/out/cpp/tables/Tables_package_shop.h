...
#ifndef SHEETMAN_GENERATED_TABLES_PACKAGE_SHOP_H
#define SHEETMAN_GENERATED_TABLES_PACKAGE_SHOP_H
...
class PackageShopTable {
 public:
  const std::vector<PackageShopRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const PackageShopRecord* find_by_id(std::int32_t key) const {
  ...
  const PackageShopRecord& get_by_id_or_throw(std::int32_t key) const {
    const PackageShopRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `PackageShop` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<PackageShopRecord> records;
    ...
    for (const sheetman::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 7: {
          sheetman::check_column(column, "PackageShop.ItemID", sheetman::kKindVarArray, 0, {sheetman::kElementI32, sheetman::kElementVarint});
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
        ...
        case 8: {
          sheetman::check_column(column, "PackageShop.ItemValue", sheetman::kKindVarArray, 0, {sheetman::kElementI32, sheetman::kElementVarint});
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = reader.read_counter32();
 ...
 private:
  ...
  void publish(std::vector<PackageShopRecord>&& records) {
  ...
  std::vector<PackageShopRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_PACKAGE_SHOP_H
