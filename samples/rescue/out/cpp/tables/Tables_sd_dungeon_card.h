...
#ifndef SHEETMAN_GENERATED_TABLES_SD_DUNGEON_CARD_H
#define SHEETMAN_GENERATED_TABLES_SD_DUNGEON_CARD_H
...
class SDDungeonCardTable {
 public:
  const std::vector<SDDungeonCardRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const SDDungeonCardRecord* find_by_id(std::int32_t key) const {
  ...
  const SDDungeonCardRecord& get_by_id_or_throw(std::int32_t key) const {
    const SDDungeonCardRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `SDDungeonCard` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<SDDungeonCardRecord> records;
 ...
 private:
  ...
  void publish(std::vector<SDDungeonCardRecord>&& records) {
  ...
  std::vector<SDDungeonCardRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_SD_DUNGEON_CARD_H
