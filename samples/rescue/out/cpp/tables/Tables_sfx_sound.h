...
#ifndef SHEETMAN_GENERATED_TABLES_SFX_SOUND_H
#define SHEETMAN_GENERATED_TABLES_SFX_SOUND_H
...
class SFXSoundTable {
 public:
  const std::vector<SFXSoundRecord>& records() const { return records_; }

  /// The row with this Id, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const SFXSoundRecord* find_by_id(std::int32_t key) const {
  ...
  const SFXSoundRecord& get_by_id_or_throw(std::int32_t key) const {
    const SFXSoundRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `SFXSound` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<SFXSoundRecord> records;
 ...
 private:
  ...
  void publish(std::vector<SFXSoundRecord>&& records) {
  ...
  std::vector<SFXSoundRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_id_;
};

}  // namespace rescue

#endif  // SHEETMAN_GENERATED_TABLES_SFX_SOUND_H
