...
{
    [System.Serializable]
    public partial class CostCurveTable
    {
        #region Record
        [System.Serializable]
        ...
        public Record GetByIdOrThrow(int key)
        {
            if (!_recordsById.TryGetValue(key, out Record record))
                throw new SheetManException($"There is no record in table `CostCurve` that corresponds to field `Id` value {key}");
                ...
                switch (column.Tag)
                {
                    ...
                    case 4:
                        ...
                        for (int i = 0; i < count; )
                        {
                            ...
                            {
                                var record = records[i++];
                                record._costType = (global::Rescue.Tables.CurrencyType)value;
                            } while (--n > 0);
                        }
                        break;
