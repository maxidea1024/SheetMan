...
class TraitTable {
     ...
     */
    var records: MutableList<TraitRecord> = ArrayList()
        private set

    private var byId: HashMap<Int, TraitRecord> = HashMap()
     ...
     */
    fun findById(key: Int): TraitRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): TraitRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<TraitRecord>(count)
        val loadedById = HashMap<Int, TraitRecord>(count * 2)

        repeat(count) { loaded.add(TraitRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                ...
                6 -> {
                    checkColumn(column, "Trait.UnlockCondition", KIND_VAR_ARRAY, 0, ELEMENT_I32, ELEMENT_VARINT)
                    for (record in loaded) {
