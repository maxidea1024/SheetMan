...
pub struct CharacterTranscendenceRecord {
    /// Transcend_ID
    pub id: i32,
    /// 영문
    pub name: String,
...
impl CharacterTranscendenceRecord {
}

/// Every row of CharacterTranscendence.
#[derive(Clone, Debug, Default)]
pub struct CharacterTranscendenceTable {
    records: Vec<CharacterTranscendenceRecord>,
    by_id: HashMap<i32, usize>,
}

impl CharacterTranscendenceTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[CharacterTranscendenceRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&CharacterTranscendenceRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&CharacterTranscendenceRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "CharacterTranscendence",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![CharacterTranscendenceRecord::default(); count];
