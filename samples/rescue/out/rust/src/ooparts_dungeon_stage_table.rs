...
pub struct OopartsDungeonStageRecord {
...
impl OopartsDungeonStageRecord {
}

/// Every row of OopartsDungeonStage.
#[derive(Clone, Debug, Default)]
pub struct OopartsDungeonStageTable {
    records: Vec<OopartsDungeonStageRecord>,
    by_id: HashMap<i32, usize>,
}

impl OopartsDungeonStageTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[OopartsDungeonStageRecord] {
        &self.records
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&OopartsDungeonStageRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> sheetman::Result<&OopartsDungeonStageRecord> {
        self.by_id
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| sheetman::Error::RecordNotFound {
                table: "OopartsDungeonStage",
                field: "Id",
                key: format!("{:?}", key),
            })
    }
    ...
    pub fn read(&mut self, filename: &Path) -> sheetman::Result<()> {
        ...
        let mut records = vec![OopartsDungeonStageRecord::default(); count];
