...
export class CharacterTranscendenceRecord {
  /** Default constructor */
  constructor() {
  }
...
// Generated from samples/rescue/MasterData.xlsx : CharacterTranscendenceTable : A1
export class CharacterTranscendenceTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): CharacterTranscendenceRecord[] { return this._records }
  private _records: CharacterTranscendenceRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, CharacterTranscendenceRecord> { return this._recordsById }
  private _recordsById: Map<number, CharacterTranscendenceRecord> = new Map<number, CharacterTranscendenceRecord>()
   ...
   */
  public findById(key: number): CharacterTranscendenceRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CharacterTranscendenceRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "CharacterTranscendence" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: CharacterTranscendenceRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CharacterTranscendenceRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CharacterTranscendenceRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        ...
        case 8:
          ...
          break
        case 9:
          sheetman.checkColumn(column, 'CharacterTranscendence.PowerMultiplier', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'CharacterTranscendence.PowerMultiplier')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: CharacterTranscendenceRecord[]): void {
    const recordsById = new Map<number, CharacterTranscendenceRecord>()
