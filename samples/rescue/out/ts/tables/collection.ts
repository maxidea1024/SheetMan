...
export class CollectionTable {
  ...
  /** All records. */
  public get records(): CollectionRecord[] { return this._records }
  private _records: CollectionRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, CollectionRecord> { return this._recordsById }
  private _recordsById: Map<number, CollectionRecord> = new Map<number, CollectionRecord>()
   ...
   */
  public findById(key: number): CollectionRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CollectionRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Collection" that corresponds to field "id" value ${key}`)
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CollectionRecord())
   ...
   */
  private publish(records: CollectionRecord[]): void {
    const recordsById = new Map<number, CollectionRecord>()
