...
export class ConditionTable {
  ...
  /** All records. */
  public get records(): ConditionRecord[] { return this._records }
  private _records: ConditionRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, ConditionRecord> { return this._recordsById }
  private _recordsById: Map<number, ConditionRecord> = new Map<number, ConditionRecord>()
   ...
   */
  public findById(key: number): ConditionRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): ConditionRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Condition" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new ConditionRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new ConditionRecord())
   ...
   */
  private publish(records: ConditionRecord[]): void {
    const recordsById = new Map<number, ConditionRecord>()
