...
export class CurrencyTable {
  ...
  /** All records. */
  public get records(): CurrencyRecord[] { return this._records }
  private _records: CurrencyRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, CurrencyRecord> { return this._recordsById }
  private _recordsById: Map<number, CurrencyRecord> = new Map<number, CurrencyRecord>()
   ...
   */
  public findById(key: number): CurrencyRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CurrencyRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Currency" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CurrencyRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CurrencyRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        ...
        case 13:
          ...
          break
        case 14:
          sheetman.checkColumn(column, 'Currency.ShortCutIndex', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: CurrencyRecord[]): void {
    const recordsById = new Map<number, CurrencyRecord>()
