...
export class GachaCharacterListRecord {
  /** Default constructor */
  constructor() {
  }
...
// Generated from samples/rescue/GachaData.xlsx : GachaCharacterListTable : A1
export class GachaCharacterListTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): GachaCharacterListRecord[] { return this._records }
  private _records: GachaCharacterListRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, GachaCharacterListRecord> { return this._recordsById }
  private _recordsById: Map<number, GachaCharacterListRecord> = new Map<number, GachaCharacterListRecord>()
   ...
   */
  public findById(key: number): GachaCharacterListRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): GachaCharacterListRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "GachaCharacterList" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: GachaCharacterListRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new GachaCharacterListRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new GachaCharacterListRecord())
   ...
   */
  private publish(records: GachaCharacterListRecord[]): void {
    const recordsById = new Map<number, GachaCharacterListRecord>()
