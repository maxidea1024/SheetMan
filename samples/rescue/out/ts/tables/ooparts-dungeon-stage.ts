...
export class OopartsDungeonStageRecord {
  /** Default constructor */
  constructor() {
  }
...
// Generated from samples/rescue/DailyDungeon.xlsx : OopartsDungeonStageTable : A1
export class OopartsDungeonStageTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): OopartsDungeonStageRecord[] { return this._records }
  private _records: OopartsDungeonStageRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, OopartsDungeonStageRecord> { return this._recordsById }
  private _recordsById: Map<number, OopartsDungeonStageRecord> = new Map<number, OopartsDungeonStageRecord>()
   ...
   */
  public findById(key: number): OopartsDungeonStageRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): OopartsDungeonStageRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "OopartsDungeonStage" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: OopartsDungeonStageRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new OopartsDungeonStageRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new OopartsDungeonStageRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        ...
        case 6:
          ...
          break
        case 7:
          sheetman.checkColumn(column, 'OopartsDungeonStage.SpawnIds', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 8:
          sheetman.checkColumn(column, 'OopartsDungeonStage.BossID', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = reader.readCounter32()
            record._bossID = []
   ...
   */
  private publish(records: OopartsDungeonStageRecord[]): void {
    const recordsById = new Map<number, OopartsDungeonStageRecord>()
