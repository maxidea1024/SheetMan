...
export class RelicDungeonStageRecord {
  /** Default constructor */
  constructor() {
  }
...
// Generated from samples/rescue/DailyDungeon.xlsx : RelicDungeonStageTable : A1
export class RelicDungeonStageTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): RelicDungeonStageRecord[] { return this._records }
  private _records: RelicDungeonStageRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, RelicDungeonStageRecord> { return this._recordsById }
  private _recordsById: Map<number, RelicDungeonStageRecord> = new Map<number, RelicDungeonStageRecord>()
   ...
   */
  public findById(key: number): RelicDungeonStageRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): RelicDungeonStageRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "RelicDungeonStage" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: RelicDungeonStageRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new RelicDungeonStageRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new RelicDungeonStageRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        ...
        case 6:
          ...
          break
        case 7:
          sheetman.checkColumn(column, 'RelicDungeonStage.SpawnIds', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 8:
          sheetman.checkColumn(column, 'RelicDungeonStage.BossID', sheetman.KIND_VAR_ARRAY, 0, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = reader.readCounter32()
            record._bossID = []
        ...
        case 10:
          ...
          break
        case 11:
          sheetman.checkColumn(column, 'RelicDungeonStage.SpawnNextTime', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'RelicDungeonStage.SpawnNextTime')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
        ...
        case 14:
          ...
          break
        case 15:
          sheetman.checkColumn(column, 'RelicDungeonStage.AtkUpPercent', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'RelicDungeonStage.AtkUpPercent')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 16:
          sheetman.checkColumn(column, 'RelicDungeonStage.DefUpPercent', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'RelicDungeonStage.DefUpPercent')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 17:
          sheetman.checkColumn(column, 'RelicDungeonStage.MHPUpPercent', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'RelicDungeonStage.MHPUpPercent')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: RelicDungeonStageRecord[]): void {
    const recordsById = new Map<number, RelicDungeonStageRecord>()
