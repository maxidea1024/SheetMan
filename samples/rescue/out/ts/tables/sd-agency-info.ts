...
export class SDAgencyInfoRecord {
  /** Default constructor */
  constructor() {
  }
...
// Generated from samples/rescue/SD_Contens.xlsx : SDAgencyInfoTable : A1
export class SDAgencyInfoTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): SDAgencyInfoRecord[] { return this._records }
  private _records: SDAgencyInfoRecord[] = []

  // Indexing by 'id'
  public get recordsById(): Map<number, SDAgencyInfoRecord> { return this._recordsById }
  private _recordsById: Map<number, SDAgencyInfoRecord> = new Map<number, SDAgencyInfoRecord>()
   ...
   */
  public findById(key: number): SDAgencyInfoRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): SDAgencyInfoRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "SDAgencyInfo" that corresponds to field "id" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: SDAgencyInfoRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new SDAgencyInfoRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new SDAgencyInfoRecord())
   ...
   */
  private publish(records: SDAgencyInfoRecord[]): void {
    const recordsById = new Map<number, SDAgencyInfoRecord>()
