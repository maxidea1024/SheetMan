...
static bool A_TemplateParse(A_TemplateTable_t* table, sm_reader* reader) {
  ...
  if (table->count > 0) {
    ...
    table->by_operator = (sm_string_index_entry*)sm_arena_alloc(
      &table->arena, (size_t)table->count * sizeof *table->by_operator);

    if (table->by_operator == NULL)
      return sm_fail_with(reader, "out of memory allocating the Operator index");
  ...
  sm_string_index_sort(table->by_operator, table->count);
...
const A_TemplateRecord_t* A_TemplateFindByOperator(
  ...
  int32_t position = sm_string_index_find(table->by_operator, table->count, key);
...
bool A_TemplateContainsOperator(
  const A_TemplateTable_t* table, const char* key) {
  return sm_string_index_find(table->by_operator, table->count, key) >= 0;
}
