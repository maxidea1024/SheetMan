...
func Update(ctx context.Context, baseURL, cacheDirectory string, options UpdateOptions) (UpdateResult, error) {
	...
	if len(wanted) == 0 && len(gone) == 0 {
		options.Log("sheetman: already up to date.")

		result.UpToDate = true
		return result, nil
	}

	options.Log(fmt.Sprintf("sheetman: %d file(s) to fetch, %d to remove.", len(wanted), len(gone)))
	...
	options.Log(fmt.Sprintf("sheetman: updated. %d fetched, %d removed.",
...
func download(ctx context.Context, url string, options UpdateOptions) ([]byte, error) {
	...
	for attempt := 1; ; attempt++ {
		...
		options.Log(fmt.Sprintf("sheetman: %v Retrying in %v (%d of %d).",
