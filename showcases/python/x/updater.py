...
def update(base_url, cache_directory, options=None):
    ...
    try:
        ...
        log("sheetman: %d file(s) to fetch, %d to remove." % (len(wanted), len(gone)))
        ...
        log("sheetman: updated. %d fetched, %d removed."
        ...
        log("sheetman: update failed: %s" % result.error)
        return result
...
def _download(url, options, log):
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            log("sheetman: %s Retrying in %.1fs (%d of %d)." % (error, delay, attempt, attempts))
