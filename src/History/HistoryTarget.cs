...
namespace SheetMan.History
{
    ...
    /// <summary>
    /// Records what this conversion changed, so the question "누가 언제 무엇을 수정했다" has
    /// an answer.
    ///
    ...
    /// </summary>
    [SheetManTarget("history", TargetKind.Description, Order = 20)]
    public class HistoryTarget : Target<HistoryRecipe>
    {
        ...
        private static void Unreachable(HistoryRecipe recipe, string section, Exception ex)
        {
            ...
            if (string.Equals(recipe.OnFailure, "fail", StringComparison.OrdinalIgnoreCase))
                throw new SheetManException(message);
