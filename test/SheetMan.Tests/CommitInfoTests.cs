...
namespace SheetMan.Tests
{
    ...
    public class CommitInfoTests : IDisposable
    {
        ...
        {
            _directory = Path.Combine(Path.GetTempPath(), "sheetman-commit-" + Guid.NewGuid().ToString("N"));
        ...
        {
            var resolved = CommitInfo.Resolve(
                WithCommitOptions(repository: _directory, commit: "x", author: "서재형 <jhseo@motifgames.com>"),
                new RecipeModel());

            Assert.Equal("서재형", resolved.AuthorName);
            Assert.Equal("jhseo@motifgames.com", resolved.AuthorEmail);
        }
        ...
        public void The_head_commit_and_its_author_are_read_from_the_working_copy()
        {
            ...
            MakeRepository("서재형", "jhseo@motifgames.com", "테이블 추가");
