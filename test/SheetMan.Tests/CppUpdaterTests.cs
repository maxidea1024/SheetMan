...
///
/// The file under test is the shipped one - lib/cpp/sheetman/updater.h - built with the
/// same one link flag a consumer needs. It is a sibling of the C one rather than a
/// wrapper around it, for the same reason the readers are siblings: a C++ consumer gets
/// std::filesystem and std::string, not char buffers and an arena.
///
...
/// </summary>
public class CppUpdaterTests : IDisposable
{
    ...
    public void The_md5_matches_the_published_vectors()
    {
        ...
        string work = Stage();
        var run = CppToolchain.RunHarness(work, "cpp-updater", server.BaseUrl,
    ...
    private string Stage()
    {
        ...
        string work = TemporaryDirectory("cpp");

        var built = CppToolchain.CompileUpdaterHarness(
            work,
            Path.Combine(RepoLayout.Root, "test", "fixtures", "tools", "cpp-updater", "main.cpp"),
            "cpp-updater");
    ...
    {
        var run = CppToolchain.RunHarness(work, "cpp-updater", baseUrl, cache);
    ...
    private string TemporaryDirectory(string name)
    {
        string directory = Path.Combine(
            RepoLayout.OutputDir("_cppupdater"),
