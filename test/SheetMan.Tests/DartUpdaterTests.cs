...
/// <summary>
/// The Dart updater, run by a real SDK against a real HTTP server.
///
/// The file under test is the shipped one - lib/dart/sheetman/updater.dart - copied
/// into a work directory and imported exactly as a consumer's program would import it.
/// The C# updater has the fuller suite; this one checks that another implementation of
/// the same design behaves the same way, which is the thing that goes wrong when a
/// design is written more than once.
///
/// It also checks the MD5 in that file, without asking about it: every download is
/// refused unless the digest matches what the exporter's own MD5 wrote.
/// </summary>
public class DartUpdaterTests : IDisposable
{
    ...
    private string Stage()
    {
        Assert.True(ConformanceHarness.DartIsAvailable(out string why),
            $"A Dart SDK is required to run the Dart updater. {why}");

        string work = TemporaryDirectory("dart");

        Directory.CreateDirectory(Path.Combine(work, "sheetman"));

        File.Copy(Path.Combine(RepoLayout.Root, "lib", "dart", "sheetman", "updater.dart"),
                  Path.Combine(work, "sheetman", "updater.dart"), overwrite: true);

        File.Copy(Path.Combine(RepoLayout.Root, "test", "fixtures", "tools", "dart-updater", "main.dart"),
                  Path.Combine(work, "main.dart"), overwrite: true);
    ...
    {
        var run = ConformanceHarness.RunDartScript(work, "main.dart", baseUrl, cache);
    ...
    private string TemporaryDirectory(string name)
    {
        string directory = Path.Combine(
            RepoLayout.OutputDir("_dartupdater"),
