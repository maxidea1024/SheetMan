...
public class DatabaseExportTests
{
    ...
    private static void RunExport()
    {
        ...
        var result = SheetManRunner.Convert(Scenario, DatabaseFixture.ConverterEnvironment);
    ...
    public void MySql_receives_every_table_with_correct_values()
    {
        ...
        Assert.False(MySqlTableExists(connection, "sm_Item__sheetman_new"),
            "A shadow table was left behind after the swap.");
        Assert.True(MySqlTableExists(connection, "sm_Item"));

        using var command = connection.CreateCommand();
        command.CommandText =
            "SELECT name, category_id, price FROM `sm_Item` ORDER BY `index`";
    ...
    public void MySql_stores_arrays_as_json()
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT tags, slot_array FROM `sm_ArrayTypes` WHERE `index` = 1";
    ...
    public void MySql_makes_the_index_field_the_primary_key()
    {
        ...
        command.CommandText =
            ...
            "WHERE table_schema = DATABASE() AND table_name = 'sm_Item' AND constraint_name = 'PRIMARY'";
    ...
    public void PostgreSql_receives_every_table_with_correct_types()
    {
        ...
        using var command = new NpgsqlCommand(
            ...
            "FROM sheetman.\"sm_TestFieldTypes\" WHERE \"index\" = 1", connection);
    ...
    public void PostgreSql_leaves_no_shadow_table_behind()
    {
        ...
        using var command = new NpgsqlCommand(
            "SELECT COUNT(*) FROM information_schema.tables " +
            "WHERE table_schema = 'sheetman' AND table_name LIKE '%__sheetman_new'", connection);
    ...
    public void PostgreSql_stores_arrays_as_jsonb()
    {
        ...
        using var command = new NpgsqlCommand(
            "SELECT jsonb_array_length(tags) FROM sheetman.\"sm_ArrayTypes\" WHERE \"index\" = 1", connection);
    ...
    public void Mongo_receives_documents_keyed_by_the_index_field()
    {
        ...
        var database = new MongoClient(DatabaseFixture.MongoConnectionString)
            .GetDatabase("sheetman_test");

        var items = database.GetCollection<BsonDocument>("sm_Item");
    ...
    public void Mongo_stores_arrays_as_bson_arrays()
    {
        ...
        var row = database.GetCollection<BsonDocument>("sm_ArrayTypes")
        ...
        var empty = database.GetCollection<BsonDocument>("sm_ArrayTypes")
    ...
    public void Mongo_leaves_no_shadow_collection_behind()
    {
        ...
        Assert.DoesNotContain(names, n => n.EndsWith("__sheetman_new"));
        Assert.Contains("sm_Item", names);
    }
    ...
    public void Redis_receives_one_hash_per_row_plus_an_index_set()
    {
        ...
        Assert.Equal("Leather Armor", (string)database.HashGet("sm_Item:2", "name"));

        // The index set is what makes "every row of this table" answerable without
        // a key scan.
        var indexed = database.SetMembers("sm_Item:index").Select(v => (string)v).OrderBy(v => v).ToArray();
    ...
    public void Redis_leaves_no_shadow_keys_behind()
    {
        ...
        Assert.Empty(server.Keys(0, pattern: "*__sheetman_new*").ToList());
    }
    ...
    public void Running_twice_replaces_rather_than_duplicates()
    {
        ...
        command.CommandText = "SELECT COUNT(*) FROM `sm_Item`";
        Assert.Equal(3L, Convert.ToInt64(command.ExecuteScalar()));

        var mongo = new MongoClient(DatabaseFixture.MongoConnectionString).GetDatabase("sheetman_test");
        Assert.Equal(3L, mongo.GetCollection<BsonDocument>("sm_Item").CountDocuments(FilterDefinition<BsonDocument>.Empty));

        using var redis = ConnectionMultiplexer.Connect(DatabaseFixture.RedisConnectionString);
        Assert.Equal(3L, redis.GetDatabase(0).SetLength("sm_Item:index"));
    }
