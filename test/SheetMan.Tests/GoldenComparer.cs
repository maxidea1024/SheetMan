...
{
    /// <summary>
    /// Set SHEETMAN_UPDATE_GOLDEN=1 to overwrite the golden tree with the current
    ...
    public static void Verify(string scenario, string goldenScenario)
    {
        ...
        if (failures.Count > 0)
        {
            throw new GoldenMismatchException(
                ...
                Environment.NewLine + Environment.NewLine +
                "If the change is intended, re-run with SHEETMAN_UPDATE_GOLDEN=1 and review the git diff.");
        }
    }
