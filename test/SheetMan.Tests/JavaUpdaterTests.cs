...
{
    /// <summary>
    /// The Java updater, compiled by javac and run on a JVM against a real HTTP server.
    ///
    /// The file under test is the shipped one - lib/java/sheetman/SheetManUpdater.java -
    /// compiled in a work directory exactly as a consumer's project would compile it. The
    /// C# updater has the fuller suite; this one checks that another implementation of the
    /// same design behaves the same way, which is the thing that goes wrong when a design
    /// is written more than once.
    ///
    /// It also exercises the JSON reader in that file, which exists because the JDK has
    /// none: the manifest it parses is the one the exporter actually wrote.
    /// </summary>
    public class JavaUpdaterTests : IDisposable
    {
        ...
        private string Stage()
        {
            Assert.True(ConformanceHarness.JavaIsAvailable(out string why),
                $"A JDK is required to run the Java updater. {why}");

            string work = TemporaryDirectory("java");

            Directory.CreateDirectory(Path.Combine(work, "sheetman"));

            File.Copy(Path.Combine(RepoLayout.Root, "lib", "java", "sheetman", "SheetManUpdater.java"),
                      Path.Combine(work, "sheetman", "SheetManUpdater.java"), overwrite: true);

            File.Copy(Path.Combine(RepoLayout.Root, "test", "fixtures", "tools", "java-updater", "Main.java"),
                      Path.Combine(work, "Main.java"), overwrite: true);

            var built = ConformanceHarness.CompileJavaSources(work);
        ...
        {
            var run = ConformanceHarness.RunJavaMain(work, baseUrl, cache);
        ...
        private string TemporaryDirectory(string name)
        {
            string directory = Path.Combine(
                RepoLayout.OutputDir("_javaupdater"),
