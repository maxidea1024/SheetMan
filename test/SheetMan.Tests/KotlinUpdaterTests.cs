...
/// <summary>
/// The Kotlin updater, compiled by kotlinc and run on a JVM against a real HTTP server.
///
/// The file under test is the shipped one - lib/kotlin/sheetman/SheetManUpdater.kt -
/// compiled in a work directory exactly as a consumer's project would compile it. It is
/// a Kotlin file rather than a call into the Java updater, so this is a fifth
/// implementation of the design and not a wrapper - and therefore worth its own gate.
/// </summary>
public class KotlinUpdaterTests : IDisposable
{
    ...
    private string Stage()
    {
        Assert.True(ConformanceHarness.KotlinIsAvailable(out string why),
            $"A Kotlin compiler is required to run the Kotlin updater. {why}");

        string work = TemporaryDirectory("kotlin");

        Directory.CreateDirectory(Path.Combine(work, "sheetman"));

        File.Copy(Path.Combine(RepoLayout.Root, "lib", "kotlin", "sheetman", "SheetManUpdater.kt"),
                  Path.Combine(work, "sheetman", "SheetManUpdater.kt"), overwrite: true);

        File.Copy(Path.Combine(RepoLayout.Root, "test", "fixtures", "tools", "kotlin-updater", "Main.kt"),
                  Path.Combine(work, "Main.kt"), overwrite: true);

        var built = ConformanceHarness.CompileKotlinJar(work, "updater.jar");
    ...
    {
        var run = ConformanceHarness.RunJar(work, "updater.jar", baseUrl, cache);
    ...
    private string TemporaryDirectory(string name)
    {
        string directory = Path.Combine(
            RepoLayout.OutputDir("_kotlinupdater"),
