...
{
    /// <summary>
    /// The Ruby updater, run by a real interpreter against a real HTTP server.
    ///
    /// The file under test is the shipped one - lib/ruby/sheetman/updater.rb - copied into
    /// a work directory and required exactly as a consumer's program would require it. The
    ...
    /// </summary>
    public class RubyUpdaterTests : IDisposable
    {
        ...
        private string Stage()
        {
            Assert.True(ConformanceHarness.RubyIsAvailable(out string why),
                $"A Ruby interpreter is required to run the Ruby updater. {why}");

            string work = TemporaryDirectory("ruby");

            Directory.CreateDirectory(Path.Combine(work, "sheetman"));

            File.Copy(Path.Combine(RepoLayout.Root, "lib", "ruby", "sheetman", "updater.rb"),
                      Path.Combine(work, "sheetman", "updater.rb"), overwrite: true);

            File.Copy(Path.Combine(RepoLayout.Root, "test", "fixtures", "tools", "ruby-updater", "main.rb"),
                      Path.Combine(work, "main.rb"), overwrite: true);
        ...
        {
            var run = ConformanceHarness.RunRubyScript(work, "main.rb", baseUrl, cache);
        ...
        private string TemporaryDirectory(string name)
        {
            string directory = Path.Combine(
                RepoLayout.OutputDir("_rubyupdater"),
