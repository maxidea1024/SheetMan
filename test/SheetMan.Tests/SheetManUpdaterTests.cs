...
namespace SheetMan.Tests
{
    ...
    ///
    /// The file under test is the one consumers get - lib/cs/sheetman/SheetManUpdater.cs is
    ...
    /// </summary>
    public class SheetManUpdaterTests : IDisposable
    {
        ...
        public async Task A_first_update_downloads_every_file()
        {
            ...
            var result = await SheetManUpdater.UpdateAsync(server.BaseUrl, cache);
        ...
        public async Task A_transient_failure_is_retried()
        {
            ...
            var result = await SheetManUpdater.UpdateAsync(server.BaseUrl, cache,
                new SheetManUpdateOptions { RetryDelay = TimeSpan.FromMilliseconds(1) });
        ...
        public async Task Retries_are_bounded()
        {
            ...
            var result = await SheetManUpdater.UpdateAsync(server.BaseUrl, cache,
                new SheetManUpdateOptions { MaxAttempts = 3, RetryDelay = TimeSpan.FromMilliseconds(1) });
        ...
        public async Task A_cancelled_update_stops()
        {
            ...
            var result = await SheetManUpdater.UpdateAsync(
        ...
        public void The_manifest_parser_reads_what_the_exporter_writes()
        {
            ...
            var manifest = SheetManManifest.Parse(File.ReadAllText(path));
        ...
        private TestServer Serve(out string servedDirectory)
        {
