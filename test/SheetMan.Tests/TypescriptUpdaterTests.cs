...
namespace SheetMan.Tests
{
    ...
    ///
    /// The file under test is the shipped one - lib/ts/sheetman/updater.ts - copied into a
    ...
    public class TypescriptUpdaterTests : IDisposable
    {
        ...
        private string Build()
        {
            ...
            File.Copy(Path.Combine(RepoLayout.Root, "lib", "ts", "sheetman", "updater.ts"),
                      Path.Combine(work, "sheetman", "updater.ts"));
...
");
  ...
  ""include"": [""main.ts"", ""ambient.d.ts"", ""sheetman/**/*.ts""]
}");
