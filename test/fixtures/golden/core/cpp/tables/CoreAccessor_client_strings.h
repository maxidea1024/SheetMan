...
class ClientStringsTable {
 public:
  ...
  const ClientStringsRecord* find_by_index(std::int32_t key) const {
  ...
  const ClientStringsRecord& get_by_index_or_throw(std::int32_t key) const {
    const ClientStringsRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw sheetman::RecordNotFound(
        std::string("there is no record in table `ClientStrings` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<ClientStringsRecord> records;
 ...
 private:
  ...
  void publish(std::vector<ClientStringsRecord>&& records) {
