...
namespace SheetMan.Binary
{
    ...
    /// </summary>
    public sealed class SheetManUpdateOptions
    {
        /// <summary>
    ...
    /// What an update did.
    /// </summary>
    public sealed class SheetManUpdateResult
    {
    ...
    public static class SheetManUpdater
    {
        ...
        {
#if UNITY_5_3_OR_NEWER
            get { return Path.Combine(UnityEngine.Application.persistentDataPath, "sheetman-data"); }
#else
            get { return Path.Combine(AppContext.BaseDirectory, "sheetman-data"); }
#endif
        }
        ...
        /// </remarks>
        public static async Task<SheetManUpdateResult> UpdateAsync(
            string baseUrl,
            string cacheDirectory = null,
            SheetManUpdateOptions options = null,
            CancellationToken cancellationToken = default)
        {
            options = options ?? new SheetManUpdateOptions();

            string cache = string.IsNullOrEmpty(cacheDirectory) ? DefaultCacheDirectory : cacheDirectory;
            var result = new SheetManUpdateResult { LocalPath = cache };
            ...
            try
            {
                ...
                var remote = SheetManManifest.Parse(remoteText);
                var local = SheetManManifest.Read(Path.Combine(cache, options.ManifestFileName));

                var wanted = new List<SheetManManifest.Entry>();
                long wantedBytes = 0;
                ...
                {
                    Log(options, "[SheetManUpdater] Already up to date.");
                ...
                Log(options, $"[SheetManUpdater] {wanted.Count} file(s) to fetch ({wantedBytes} bytes), " +
                ...
                foreach (var entry in wanted)
                {
                    ...
                    if (options.VerifyHash && !string.IsNullOrEmpty(entry.Hash))
                    {
                        ...
                        {
                            throw new SheetManUpdateException(
                ...
                Log(options, $"[SheetManUpdater] Updated. {result.DownloadedCount} fetched, " +
            ...
            catch (Exception e)
            {
                ...
                Log(options, $"[SheetManUpdater] Update failed: {e.Message}");
        ...
        /// </remarks>
        private static bool IsCurrent(SheetManManifest local, SheetManManifest.Entry entry, string cache)
        {
        ...
        private static async Task<byte[]> DownloadAsync(
            string url, SheetManUpdateOptions options, CancellationToken cancellationToken)
        {
            ...
            for (int attempt = 1; ; attempt++)
            {
                ...
                catch (SheetManUpdateException e) when (e.IsTransient && attempt < attempts)
                {
                    Log(options, $"[SheetManUpdater] {e.Message} Retrying in {delay.TotalMilliseconds:F0}ms " +
                                 $"({attempt} of {attempts}).");
            ...
            string url, SheetManUpdateOptions options, CancellationToken cancellationToken)
        {
            using (var request = UnityEngine.Networking.UnityWebRequest.Get(url))
            {
                ...
                bool connection = request.isNetworkError;
#endif
            ...
            string url, SheetManUpdateOptions options, CancellationToken cancellationToken)
        {
            using (var timeout = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
            {
                ...
                catch (HttpRequestException e)
                {
                    ...
                    throw new SheetManUpdateException($"'{url}' could not be reached: {e.Message}.", true);
                }
        ...
        private static void Log(SheetManUpdateOptions options, string message)
        {
            if (options.Log != null)
                options.Log(message);
        }
    ...
    /// </summary>
    public sealed class SheetManUpdateException : Exception
    {
        public SheetManUpdateException(string message, bool isTransient = false)
            : base(message)
        {
    ...
    /// </remarks>
    public sealed class SheetManManifest
    {
        ...
        /// </summary>
        public static SheetManManifest Read(string filename)
        {
            try
            {
                return File.Exists(filename)
                    ? Parse(File.ReadAllText(filename))
                    : new SheetManManifest();
            }
            catch (Exception)
            {
        ...
        public static SheetManManifest Parse(string json)
        {
            var manifest = new SheetManManifest();
