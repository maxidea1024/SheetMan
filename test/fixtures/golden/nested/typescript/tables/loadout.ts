...
export class LoadoutTable {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          sheetman.checkColumn(column, 'Loadout.Index', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'Loadout.Index')
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          sheetman.checkColumn(column, 'Loadout.Name', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'Loadout.Name')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 3:
          sheetman.checkColumn(column, 'Loadout.Pos.X', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'Loadout.Pos.X')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 4:
          sheetman.checkColumn(column, 'Loadout.Pos.Y', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_F32])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'Loadout.Pos.Y')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
          ...
          break
        case 5:
          sheetman.checkColumn(column, 'Loadout.Slot.Id', sheetman.KIND_FIXED_ARRAY, 2, [sheetman.ELEMENT_I32, sheetman.ELEMENT_VARINT])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            for (let j = 0; j < 2; ++j)
          ...
          break
        case 6:
          sheetman.checkColumn(column, 'Loadout.Slot.Label', sheetman.KIND_FIXED_ARRAY, 2, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            for (let j = 0; j < 2; ++j)
              record._slot[j].label = reader.readString()
          }
          break
        case 7:
          sheetman.checkColumn(column, 'Loadout.Note', sheetman.KIND_SCALAR, 1, [sheetman.ELEMENT_STRING])
          cursor = new sheetman.ScbColumnCursor(reader, column, rowCount, 'Loadout.Note')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._note = cursor.nextString()
          }
          break
        case 8:
          sheetman.checkColumn(column, 'Loadout.Tag_array', sheetman.KIND_FIXED_ARRAY, 2, [sheetman.ELEMENT_STRING])
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            record._tagArray = []
            for (let j = 0; j < 2; ++j)
              record._tagArray.push(reader.readString())
          }
          break
        default:
