...
/// <summary>
/// Thin authoring helper over NPOI that lays out SheetMan entities at an explicit
/// (column, row) origin, so fixtures read the same way they look in Excel.
///
/// SheetMan's layout rules (see ModelCooker.ParseDefinitionRect) are:
///
...
public sealed class SheetBuilder
{
    ...
    /// SheetMan currently reads these as raw serial numbers - see XlsxImporter.SafeCellValue.
    /// </summary>
    ...
    ///
    /// SheetMan reads cached formula results rather than evaluating anything, so the
    ...
    public int Enum(int column, int row, EnumSpec spec)
    {
        ...
        // Header row is a placeholder that SheetMan skips; it exists for human readers.
