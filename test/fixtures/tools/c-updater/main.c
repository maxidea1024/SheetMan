 ...
 *
 * The updater under test is the shipped one - lib/c/sheetman/sheetman_updater.h -
...
int main(int argc, char** argv) {
...

  {
    ...
    sm_md5_hex((const uint8_t*)"abc", 3, digest);
    printf("{\"md5abc\":\"%s\"", digest);

    sm_md5_hex((const uint8_t*)"", 0, digest);
    printf(",\"md5empty\":\"%s\"", digest);

    sm_md5_hex((const uint8_t*)fox, strlen(fox), digest);
    printf(",\"md5fox\":\"%s\"}\n", digest);
  }
  ...
  sm_update(argv[1], argv[2], &options, &result);
