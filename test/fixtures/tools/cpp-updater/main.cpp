...
//
// The updater under test is the shipped one - lib/cpp/sheetman/updater.h - compiled
...
int main(int argc, char** argv) {
  ...
  std::cout << "{\"md5abc\":\"" << sheetman::md5_hex(bytes_of("abc"))
            << "\",\"md5empty\":\"" << sheetman::md5_hex(bytes_of(""))
            << "\",\"md5fox\":\""
            << sheetman::md5_hex(bytes_of("The quick brown fox jumps over the lazy dog"))
            << "\"}\n";
  ...
  const sheetman::UpdateResult result = sheetman::update(argv[1], argv[2], options);
