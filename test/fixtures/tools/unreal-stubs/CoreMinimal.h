...
// ------------------------------------------------------------------------ logging
//
// A no-op. The generated code logs a reason for every failure and the harness fails on the
// return value, so nothing reads these - but they have to compile, and a variadic macro is the
// only shape that accepts every call the generated code makes.
